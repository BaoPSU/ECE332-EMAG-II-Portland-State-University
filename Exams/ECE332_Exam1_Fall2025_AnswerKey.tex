...
\end{tikzpicture}
\end{center}
\textbf{(a)} Find a formula for the magnetic flux through one turn of the loop.\hfill\textit{(6 pts)}\\[3pt]
...
$V_\text{emf}=-N\dfrac{d\Phi}{dt}$,\; $\Phi=\displaystyle\int_S\mathbf{B}\cdot d\mathbf{s}$ &
$y$-$z$ plane $\Rightarrow d\mathbf{s}=\hat{x}\,da$\\[5pt]
\textbf{DOT PRODUCTS} & \textbf{ROTATING LOOP areas}\\
$\hat{x}\cdot\hat{x}=1$,\quad $\hat{y}\cdot\hat{x}=0$ & Equil.\ tri.\ (side $a$): $A=\dfrac{\sqrt{3}}{4}a^2$\\[5pt]
...
\qquad\boxed{V_\text{emf}(t)=\tfrac{\sqrt{3}}{2}a^2 B_0 N\omega\cos(\omega t)\;\text{V}}\]

\textbf{(c)}
\begin{notebox}
Exam says ``$\omega=2\,\text{MHz}$'' $\Rightarrow$ convert: $\omega=2\pi f=2\pi(2\times10^6)=4\pi\times10^6\,\text{rad/s}$
\end{notebox}
\[|V_\text{emf}|=\tfrac{\sqrt{3}}{2}(0.08)^2(10)(0.02)(4\pi\times10^6)\underbrace{\cos(\pi/4)}_{\sqrt{2}/2}=9.85\times10^3\;\text{V}\]
