...
\textit{From COMMON ANGLES box: $\sin^2(45^\circ) = 1/2$.}

\textit{Simplify $k^2 = (2\pi/300)^2$:}
\[(2\pi/300)^2 = \tfrac{4\pi^2}{90000} = \tfrac{4\pi^2}{9{\times}10^4}\]

\textit{Simplify $I_0^2 = 144$, $l^2 = 1$, $R^2 = (5000)^2 = 25{\times}10^6$.}

\textit{Substitute everything:}
\[S = \tfrac{(120\pi)\cdot(4\pi^2/9{\times}10^4)\cdot 144\cdot 1}{32\pi^2\cdot 25{\times}10^6}\cdot\tfrac{1}{2}\]

\textit{Group the numerical coefficients (separate from $\pi$ powers):}

Numerator: $120\cdot 4\cdot 144 / (9{\times}10^4) = 69120/9{\times}10^4 = 0.7680$\\
With $\pi$ powers: $\pi\cdot\pi^2 = \pi^3$, so numerator $= 0.7680\,\pi^3$

Denominator: $32\cdot 25{\times}10^6 = 800{\times}10^6 = 8{\times}10^8$\\
With $\pi$ powers: $\pi^2$, so denominator $= 8{\times}10^8\,\pi^2$

\textit{Combine:}
\[S = \tfrac{0.7680\,\pi^3}{8{\times}10^8\,\pi^2}\cdot\tfrac{1}{2} = \tfrac{0.7680\,\pi}{8{\times}10^8}\cdot\tfrac{1}{2}\]
\[= \tfrac{0.7680\cdot 3.14159}{8{\times}10^8}\cdot 0.5 = \tfrac{2.413}{8{\times}10^8}\cdot 0.5 = \tfrac{1.207}{8{\times}10^8}\]
\[= 1.508{\times}10^{-9}~\text{W/m}^2\]
...
Convert rad $\to$ deg (multiply by $180/\pi$, from COMMON ANGLES box):
...
\textbf{Plot:} computed in Python (\texttt{make\_p3\_plot.py}), shown below in both rectangular and polar form.

\begin{center}
\includegraphics[width=0.85\linewidth]{HW4_P3c_rect.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.55\linewidth]{HW4_P3c_polar.png}
\end{center}
...
\includegraphics[width=0.49\linewidth]{cs_db}\\[3pt]
\includegraphics[width=0.49\linewidth]{cs_dipole_params}
\end{cssnip}
...
\textit{Step 1 --- compute $\lambda/(4\pi R)$:}
...
\textit{Convert $G_t = 20$ dB to linear} (from dB$\leftrightarrow$LINEAR box: $X_\text{lin} = 10^{X_\text{dB}/10}$):
\[G_t = 10^{20/10} = 10^2 = 100\;(\text{unitless})\]

\textit{Convert $G_r = 23$ dB to linear:}
...
\textbf{Plug in step by step:}

\textit{Numerator:}\ $G_t P_t = 100 \cdot 10 = 1000$ W

\textit{Square $R$:}\ $R^2 = (2{\times}10^4)^2 = 4{\times}10^8$ m$^2$

\textit{Denominator:}\ $4\pi R^2 = 4\pi(4{\times}10^8) = 4(3.14159)(4{\times}10^8) = 50.27{\times}10^8 = 5.027{\times}10^9$ m$^2$

\textit{Divide:}
\[S_r = \tfrac{1000}{5.027{\times}10^9} = 1.989{\times}10^{-7}~\text{W/m}^2\]

\textit{Convert to readable unit:} $10^{-6}$ = micro ($\mu$), so $1.989{\times}10^{-7} = 0.1989{\times}10^{-6}$:
...
\textbf{Convert $\beta$ to radians} (deg $\to$ rad, multiply by $\pi/180$, from COMMON ANGLES box):
\[\beta = 3^\circ \cdot \tfrac{\pi}{180} = \tfrac{3\pi}{180} = \tfrac{\pi}{60}\]
\[= \tfrac{3.14159}{60} = 0.05236~\text{rad}\]

\textbf{Plug in:}
\[D = \tfrac{4\pi}{(0.05236)^2} = \tfrac{12.566}{0.002742} = 4584\;(\text{unitless})\]
Convert to dB using the standard linear$\to$dB formula (from dB$\leftrightarrow$LINEAR box):
\[D_{\text{dB}} = 10\log_{10}(D_{\text{lin}}) = 10\log_{10}(4584) = 10\cdot 3.661\]
\[\boxed{D_{\text{dB}} = 36.6\text{ dB}}\]

\vspace{6pt}
\textbf{(b) Area doubled $A_p \to 2A_p$ (5 pts)}

\begin{goalbox}
\goal{find new $D$ and $\beta$ by \emph{reading proportionalities} off the aperture formulas. Don't recompute --- just scale the part (a) answers.}
\need{$D_{\text{old}}\!=\!4584$ and $\beta_{\text{old}}\!=\!3^\circ$ from part (a); know which variables change.}
\end{goalbox}

\textbf{Step 1 --- write the base formulas (APERTURE ANTENNAS box):}
\[D \approx \tfrac{4\pi A_p}{\lambda^2}\;\;\;\text{and}\;\;\;\beta \approx \tfrac{1.02\,\lambda}{d}\;(\text{circular, diam.\ }d)\]
For a circular aperture $A_p\!=\!\pi d^2/4$, so $d\!=\!2\sqrt{A_p/\pi}$. Substitute into $\beta$:
\[\beta \approx \tfrac{1.02\,\lambda}{2\sqrt{A_p/\pi}} = \tfrac{0.904\,\lambda}{\sqrt{A_p}}\;\;\Rightarrow\;\;\beta \propto \tfrac{\lambda}{\sqrt{A_p}}\]

\textbf{Step 2 --- strip the constants, keep proportionalities:}
\[\boxed{D \propto \tfrac{A_p}{\lambda^2}}\;\;\;\text{and}\;\;\;\boxed{\beta \propto \tfrac{\lambda}{\sqrt{A_p}}}\]

\textbf{Step 3 --- in part (b) only $A_p$ changes} ($f$ stays the same $\Rightarrow$ $\lambda$ stays the same, so anything with $\lambda$ drops out):
\begin{itemize}\setlength{\itemsep}{0pt}
\item $D$ depends on $A_p$ \emph{linearly} $\Rightarrow$ doubling $A_p$ \emph{doubles} $D$:\ \ $D_\text{new}\!=\!2D_\text{old}$.
\item $\beta$ depends on $1/\sqrt{A_p}$ $\Rightarrow$ doubling $A_p$ \emph{shrinks} $\beta$ by $\sqrt{2}$:\ \ $\beta_\text{new}\!=\!\beta_\text{old}/\sqrt{2}$.
\end{itemize}

\textit{\color{coral}Why $\sqrt{2}$ not $2$?\ Area $A_p\!=\!\pi d^2/4$ scales with diameter squared. Doubling the area only scales the diameter by $\sqrt{2}$, and $\beta$ follows the diameter not the area.}

\textbf{Step 4 --- plug in part (a) values:}
\[D_{\text{new}} = 2(4584) = 9168 \;\Rightarrow\; 10\log_{10}(9168) = \boxed{D_{\text{new}} = 39.6~\text{dB}}\]
\[\beta_{\text{new}} = 3^\circ/\sqrt{2} = 3^\circ/1.414 = \boxed{2.12^\circ}\]

\textit{Bookkeeping version (same answer, equivalent path)} --- the general ``Scaling (any ratio)'' block from the cheat sheet handles both $A_p$ and $f$ changing at once:
\[D_{\text{new}} = D_{\text{old}} \cdot \tfrac{A_{p,\text{new}}}{A_{p,\text{old}}} \cdot \!\left(\tfrac{f_{\text{new}}}{f_{\text{old}}}\right)^{\!2},\quad \beta_{\text{new}} = \beta_{\text{old}} \cdot \sqrt{\tfrac{A_{p,\text{old}}}{A_{p,\text{new}}}} \cdot \tfrac{f_{\text{old}}}{f_{\text{new}}}\]
For (b): $A_p$ ratio $= 2$, $f$ ratio $= 1$, so it collapses to $\times 2$ for $D$ and $\div\sqrt{2}$ for $\beta$. Same answer as Step 4.
...
\begin{cssnip}
\begin{center}\includegraphics[width=0.55\linewidth]{cs_array}\end{center}
\end{cssnip}
