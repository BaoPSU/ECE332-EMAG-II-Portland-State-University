...
\begin{goalbox}
\goal{find new $D$ and $\beta$ using the scaling rules (from APERTURE ANTENNAS box, ``Scaling (any ratio)'' block):}
...
\textit{Here $f$ unchanged, $A_p \to 2A_p$, so $D \propto A_p$ and $\beta \propto 1/\sqrt{A_p}$:}
\[D_{\text{new}} = 2D_{\text{old}},\quad \beta_{\text{new}} = \beta_{\text{old}}/\sqrt{2}\]
