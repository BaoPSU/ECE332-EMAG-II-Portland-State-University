...
The constraint from the lab is $W \geq L$, with larger $W$ dropping the input impedance and broadening the pattern, and a square patch ($W = L$) sitting around 300~$\Omega$. We picked $W = \fb{TBD~mm}$ to leave room for the inset feed cuts on either side of the transmission line and to align with the long edge of the FR4 board.

\subsection*{50~\texorpdfstring{$\Omega$}{ohm} transmission line width}

For a microstrip line on FR4 with substrate height $h = 1.6$~mm and $\varepsilon_r = 4.2$, the 50~$\Omega$ trace width is approximately $w \approx 3.0$~mm (Hammerstad/Wheeler formula, or a microstrip calculator). The exact value used on our antenna was \fb{w = TBD~mm}.
...
\fb{Insert photo of Bao's antenna with L, W, R, and transmission line dimensions marked in Sharpie.}

\vspace{1em}
\fb{Insert photo of Nick's antenna with L, W, R, and transmission line dimensions marked in Sharpie.}
...
$L$ (mm)                       & \fb{TBD} & \fb{TBD} \\
$W$ (mm)                       & \fb{TBD} & \fb{TBD} \\
Transmission line width (mm)   & \fb{TBD} & \fb{TBD} \\
...
\fb{Insert VNA screen capture showing $S_{11}$ and $S_{22}$ Log-Mag and Smith plots with both antennas connected, with three markers at 2.400, 2.443, and 2.487~GHz.}
...
\fb{Discuss: is cross-pol lower than back-to-back, or higher? If lower, the patches are well-polarized and back-to-back coupling is dominated by reflections off the room. If higher, explain why (likely near-field coupling that doesn't respect polarization, or reflections off nearby surfaces). Address near-field vs far-field for the chosen separation distance: the far-field boundary for a patch with largest dimension $D$ is roughly $2D^2/\lambda$, so at 2.443~GHz with $\lambda \approx 12.3$~cm and $D \approx \fb{W}$~mm, the far-field begins at about $\fb{TBD~cm}$.}
