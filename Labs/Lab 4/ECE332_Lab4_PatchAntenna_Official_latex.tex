...
Return loss at $f_c = 2.446$~GHz (dB) & $-22.3$  & $-31.1$ \\
Return loss at 2.400~GHz (dB)      & $-6.7$   & $-10.7$ \\
Return loss at 2.487~GHz (dB)      & $-9.7$   & $-11.4$ \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Final state with markers (Deliverable \#1):} four-quadrant view of $S_{11}$/$S_{22}$ in Log-Mag and Smith, with three markers at 2.400~GHz, 2.446~GHz (center), and 2.487~GHz.
\begin{center}
\includegraphics[width=0.95\textwidth]{src/photos/vna_05_final_markers}
\end{center}

\textbf{Marker readings:}
\begin{itemize}[leftmargin=*]
  \item Marker 1 (2.400~GHz): $S_{11} = -6.7$~dB, $S_{22} = -10.7$~dB
  \item Marker 2 (2.487~GHz): $S_{11} = -9.7$~dB, $S_{22} = -11.4$~dB
  \item Marker 3 (2.446~GHz, center): $S_{11} = -22.3$~dB, $S_{22} = -31.1$~dB
\end{itemize}

\textbf{Compliance check vs the lab spec:}
\begin{itemize}[leftmargin=*]
  \item ``Below $-20$~dB at design frequency'' (deliverable \#1): $S_{11} = -22.3$~dB \textbf{PASS} and $S_{22} = -31.1$~dB \textbf{PASS}
  \item ``At least $-9.5$~dB across WiFi band (2.400--2.487~GHz)'': $S_{22}$ passes at both edges (deeper than $-10$~dB). $S_{11}$ passes at 2.487~GHz ($-9.7$~dB) but is marginal at 2.400~GHz ($-6.7$~dB), so Bao's antenna does not strictly meet the band-edge spec at the low end.
  \item Nick's $S_{22}$ at center reads $-31.1$~dB, exceeding the $-30$~dB threshold mentioned in the procedure for the bonus exam-point opportunity.
\end{itemize}
