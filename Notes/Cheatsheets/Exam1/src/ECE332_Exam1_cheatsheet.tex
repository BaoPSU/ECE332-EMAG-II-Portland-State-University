...
\[V_\text{emf} = -\frac{d\Phi}{dt}\;\text{(V)} \qquad \Phi = \int_S \mathbf{B} \cdot d\mathbf{s}\;\text{(V$\cdot$s)}\]

\eq{$N$-turn coil}{transformer, $N$ turns}
\[V_\text{emf} = -N\frac{d\Phi}{dt} = -\frac{d\lambda}{dt}\;\text{(V)} \quad \lambda=N\Phi\;\text{(V$\cdot$s)}\]
...
\[E = \frac{V(t)}{d} \qquad J_c = \sigma E\;\text{(A/m}^2\text{)} \qquad I_c = J_c \cdot A\;\text{(A)}\]
...
\[J_d = \varepsilon\frac{\partial E}{\partial t}\;\text{(A/m}^2\text{)} \qquad I_d = J_d \cdot A\;\text{(A)}\]
...
\textit{R, C from geometry only; sub $V(t)$ and $dV/dt$ last.}
\end{ebox}
...
\[\rho_v(t) = \rho_{v0}\,e^{-t/\tau_r}\;\text{(C/m}^3\text{)} \qquad \tau_r = \frac{\varepsilon_r\varepsilon_0}{\sigma}\;\text{(s)}\]
\textit{$\rho_{v0}$ (C/m$^3$);\enspace$\sigma$ (S/m);\enspace$\varepsilon_0=8.85\times10^{-12}$ F/m}
...
$\Phi=\int_S\mathbf{B}\cdot d\mathbf{s}$ — flux (V$\cdot$s)\\
$\lambda=N\Phi$ — flux linkage (V$\cdot$s)
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
...
\[\mathbf{S}_\text{av}=\frac{|\tilde{\mathbf{E}}(0)|^2}{2|\eta_c|}e^{-2\alpha z}\cos\theta_\eta\;\hat{z}\;\text{(W/m}^2\text{)}\]
