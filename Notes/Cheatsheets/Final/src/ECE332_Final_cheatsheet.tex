...
\graphicspath{{../img/}{../../Cheatsheets/Midterm1/img/}{../../../Textbook/tables/}}
...
\begin{ebox}
\eq{Loss tangent}{$\varepsilon'\!=\!\varepsilon_r\varepsilon_0$, $\varepsilon_0\!\approx\!1/(36\pi\!\times\!10^9)$}
\[\tfrac{\varepsilon''}{\varepsilon'} = \tfrac{\sigma}{\omega\varepsilon_r\varepsilon_0}\]

{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\fontsize{5.6}{6.5}\selectfont\begin{tabular}{@{}p{0.18\linewidth}p{0.12\linewidth}p{0.62\linewidth}@{}}
\toprule
Type & $\sigma/\omega\varepsilon$ & $\eta_c,\;\alpha,\;\beta$\\
\midrule
\rowcolor{rowB}Lossless ($\sigma\!=\!0$) & 0 & $\eta\!=\!\eta_0/\sqrt{\varepsilon_r}$ exact;\, $\alpha\!=\!0$,\, $\beta\!=\!\omega\sqrt{\varepsilon_r}/c$\\
...
\rowcolor{rowB}Quasi-cond. & $10^{-2}$--$10^2$ & exact: $(\eta/\sqrt{X})e^{j\theta_\eta}$, see below\\
...
\[\alpha=\omega\sqrt{\tfrac{\mu\varepsilon'}{2}}\sqrt{X{-}1},\;\beta=\omega\sqrt{\tfrac{\mu\varepsilon'}{2}}\sqrt{X{+}1},\;\theta_\eta=\tfrac{1}{2}\arctan\!\tfrac{\sigma}{\omega\varepsilon'}\]
\end{ebox}
...
\begin{ebox}
\begin{center}\includegraphics[width=0.65\linewidth]{p3_layered_diagram}\end{center}
\eq{Beam offset through $N$ slabs}{thickness $t_i$, refracted angle inside slab $i$}

\eq{Slab-indexed}{$\theta_i = $ angle inside slab $i$}
\[d = \sum_{i=1}^{N} t_i\,\tan\theta_i\]

\eq{Angle-indexed (HW3 P3 style)}{$\theta_1 = $ incident, $\theta_{i+1}=$ inside slab $i$}
\[d = \sum_{i=1}^{N} t_i\,\tan\theta_{i+1}\]
\textit{Same physics, different numbering. In HW3 P3: slab 1 contributes $t_1\tan\theta_2$, slab 2 contributes $t_2\tan\theta_3$. The angle in the formula is always the one the ray makes \emph{inside that slab}.}\\[1pt]
\textit{Air--slab--air: exit angle returns to $\theta_1$ but is laterally shifted by $d$.}
\end{ebox}
...
\begin{ebox}
\textit{Shorthand: $c_i\!=\!\cos\theta_i$, $c_t\!=\!\cos\theta_t$.}\\[1pt]
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{1pt}\fontsize{5.2}{6.0}\selectfont\begin{tabular}{@{}p{0.10\linewidth}p{0.22\linewidth}p{0.30\linewidth}p{0.30\linewidth}@{}}
\toprule
 & \textbf{Normal} ($\theta_i\!=\!0$) & $\boldsymbol{\perp}$ \textbf{pol} & $\boldsymbol{\parallel}$ \textbf{pol}\\
\midrule
\rowcolor{rowB}$\Gamma$ & $\tfrac{\eta_2-\eta_1}{\eta_2+\eta_1}$ & $\tfrac{\eta_2 c_i-\eta_1 c_t}{\eta_2 c_i+\eta_1 c_t}$ & $\tfrac{\eta_2 c_t-\eta_1 c_i}{\eta_2 c_t+\eta_1 c_i}$\\
$\tau$ & $\tfrac{2\eta_2}{\eta_2+\eta_1}$ & $\tfrac{2\eta_2 c_i}{\eta_2 c_i+\eta_1 c_t}$ & $\tfrac{2\eta_2 c_i}{\eta_2 c_t+\eta_1 c_i}$\\
\rowcolor{rowB}$\tau$ rel.\ & $\tau\!=\!1\!+\!\Gamma$ & $\tau_\perp\!=\!1\!+\!\Gamma_\perp$ & $\tau_\parallel\!=\!(1\!+\!\Gamma_\parallel)\tfrac{c_i}{c_t}$\\
$R$ & $|\Gamma|^2$ & $|\Gamma_\perp|^2$ & $|\Gamma_\parallel|^2$\\
\rowcolor{rowB}$T$ & $|\tau|^2\tfrac{\eta_1}{\eta_2}$ & $|\tau_\perp|^2\tfrac{\eta_1 c_t}{\eta_2 c_i}$ & $|\tau_\parallel|^2\tfrac{\eta_1 c_t}{\eta_2 c_i}$\\
$R\!+\!T$ & $T\!=\!1\!-\!R$ & $T_\perp\!=\!1\!-\!R_\perp$ & $T_\parallel\!=\!1\!-\!R_\parallel$\\
\bottomrule
\end{tabular}}}\par}
\textit{\color{blue}Notes: $\sin\theta_t\!=\!\sqrt{\mu_1\varepsilon_1/\mu_2\varepsilon_2}\sin\theta_i$;\ $\eta_i\!=\!\sqrt{\mu_i/\varepsilon_i}$;\ nonmag.\ $\Rightarrow\eta_2/\eta_1\!=\!n_1/n_2$.}
\end{ebox}
...
\begin{ebox}
\textit{Shorthand: $C_X\!=\!\cos\tfrac{m\pi x}{a}$, $S_X\!=\!\sin\tfrac{m\pi x}{a}$, $C_Y\!=\!\cos\tfrac{n\pi y}{b}$, $S_Y\!=\!\sin\tfrac{n\pi y}{b}$. All fields carry an $e^{-j\beta z}$ factor (omitted below). $k_c^2 = (m\pi/a)^2+(n\pi/b)^2$.}\\[1pt]

\eq{TE mode}{generator: $\tilde{H}_z$}
\[\tilde{H}_z = H_0\,C_X C_Y\]
\[\tilde{E}_x = \tfrac{j\omega\mu}{k_c^2}\!\left(\tfrac{n\pi}{b}\right) H_0\,C_X S_Y\]
\[\tilde{E}_y = -\tfrac{j\omega\mu}{k_c^2}\!\left(\tfrac{m\pi}{a}\right) H_0\,S_X C_Y\]
\[\tilde{E}_z = 0,\quad \tilde{H}_x = -\tilde{E}_y/Z_\text{TE},\quad \tilde{H}_y = \tilde{E}_x/Z_\text{TE}\]

\eq{TM mode}{generator: $\tilde{E}_z$}
\[\tilde{E}_z = E_0\,S_X S_Y\]
\[\tilde{E}_x = -\tfrac{j\beta}{k_c^2}\!\left(\tfrac{m\pi}{a}\right) E_0\,C_X S_Y\]
\[\tilde{E}_y = -\tfrac{j\beta}{k_c^2}\!\left(\tfrac{n\pi}{b}\right) E_0\,S_X C_Y\]
...
TE$_{10}$ & $u_{p0}/(2a)$ (dominant)\\
\rowcolor{rowB}TE$_{01}$ & $u_{p0}/(2b)$\\
TE$_{20}$ & $u_{p0}/a = 2f_{10}$\\
\rowcolor{rowB}TE$_{11}$,TM$_{11}$ & $\tfrac{u_{p0}}{2}\sqrt{1/a^2+1/b^2}$\\
\bottomrule
\end{tabular}}}\par}
...
% ============================================================
% APPENDIX: Textbook reference tables (Ch 7-8)
% ============================================================
\newpage
\thispagestyle{empty}
\begin{center}\Large\bfseries\color{purple}TEXTBOOK REFERENCE TABLES\end{center}
\vspace{4pt}

{\centering
\textbf{\small Table 7-1 --- Lossy media: $\alpha$, $\beta$, $\eta_c$, $u_p$, $\lambda$ by regime}\\[2pt]
\includegraphics[width=0.95\textwidth]{Table_7-1_lossy_media_alpha_beta_eta}\par}
\vspace{8pt}

{\centering
\textbf{\small Table 7-2 --- dB $\leftrightarrow$ linear power ratios}\\[2pt]
\includegraphics[width=0.40\textwidth]{Table_7-2_dB_power_ratios}\par}
\vspace{8pt}

{\centering
\textbf{\small Table 8-1 --- Normal incidence plane wave $\leftrightarrow$ transmission line analogy}\\[2pt]
\includegraphics[width=0.95\textwidth]{Table_8-1_plane_wave_transmission_line_analogy}\par}

\newpage
\thispagestyle{empty}
{\centering
\textbf{\small Table 8-2 --- $\Gamma$, $\tau$, $R$, $T$ for normal / $\perp$ / $\parallel$ polarization}\\[2pt]
\includegraphics[width=0.95\textwidth]{Table_8-2_reflection_transmission}\par}
\vspace{12pt}

{\centering
\textbf{\small Table 8-3 --- TE / TM / TEM full field components in rectangular waveguide}\\[2pt]
\includegraphics[width=0.95\textwidth]{Table_8-3_waveguide_fields}\par}
