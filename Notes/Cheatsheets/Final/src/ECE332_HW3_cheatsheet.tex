...
\[\tilde{H}_z = H_0\cos\!\tfrac{m\pi x}{a}\cos\!\tfrac{n\pi y}{b}\,e^{-j\beta z}\;\text{(A/m)}\]
\eq{TM mode}{$H_z=0$, $E_z\ne0$}
\[\tilde{E}_z = E_0\sin\!\tfrac{m\pi x}{a}\sin\!\tfrac{n\pi y}{b}\,e^{-j\beta z}\;\text{(V/m)}\]
...
\begin{ebox}
\textit{$e^{-j\beta z}$ factor omitted. $k_c^2\!=\!(m\pi/a)^2\!+\!(n\pi/b)^2$. $\tilde{E}$ in V/m, $\tilde{H}$ in A/m.}\\[1pt]
...
\begin{ebox}
\[Z_{\text{TE}} = \tfrac{\eta}{\sqrt{1-(f_c/f)^2}}\,(\Omega)\quad Z_{\text{TM}} = \eta\sqrt{1-(f_c/f)^2}\,(\Omega)\]
\[\eta = \tfrac{\eta_0}{\sqrt{\varepsilon_r}}\,(\Omega) \quad H_y = \tfrac{E_x}{Z_{\text{TE}}}\,(\text{A/m, TE mode})\]
