...
%-- Column 1 -----------------------------------
\shead{purplebg}{purple}{INTRINSIC IMPEDANCE $\eta$}
\begin{ebox}
\textit{\color{coral}\textbf{Med.\ 1} ($z<0$): incident side.\enspace\textbf{Med.\ 2} ($z\ge0$): transmitted side. Boundary at $z=0$. ($\Gamma, \tau$ in Table 8-2 below.)}

\eq{General $\to$ nonmag.\ shortcut}{}
...
\eq{Low-loss correction}{$\sigma/(\omega\varepsilon)\ll1$}
\[\eta_c \approx \sqrt{\tfrac{\mu}{\varepsilon'}}\!\left(1+j\tfrac{\sigma}{2\omega\varepsilon'}\right) \;\xrightarrow{\text{drop tiny }j}\; \tfrac{\eta_0}{\sqrt{\varepsilon_r}}\;(\Omega)\]
\textit{\color{coral}For $\Gamma$/$\tau$ just use $\eta_0/\sqrt{\varepsilon_r}$. The $j$ term is for accuracy with $|\eta_c|, \theta_\eta$. See LOSSY MEDIA --- 5 CASES table for all regimes.}
\end{ebox}
