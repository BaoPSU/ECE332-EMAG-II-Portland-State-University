...
\[\tilde{E}_x = E_0\,e^{-j\beta z},\;\; \tilde{E}_y = \tilde{E}_z = 0\]
\[\tilde{H}_y = \tilde{E}_x/\eta,\;\; \tilde{H}_x = \tilde{H}_z = 0\]
...
\[u_{p0} = 1/\sqrt{\mu\varepsilon}\;(\text{m/s})\]
\end{ebox}
...
\begin{ebox}
\eq{Practical}{$\eta_0\!=\!120\pi$; nonmag.\ media}
\[\eta = \tfrac{\eta_0}{\sqrt{\varepsilon_r}}\,(\Omega)\quad(\!\Leftrightarrow\!\sqrt{\mu/\varepsilon}\text{ in Tab.\ 8-3})\]
\[u_g = u_{p0}\sqrt{1-(f_{mn}/f)^2}\;\text{(m/s)}\]
\[u_p u_g = u_{p0}^2\;\text{(m}^2\text{/s}^2)\]
\[t = L/u_g\;\text{(s)}\quad\text{travel time through guide of length }L\]
\textit{Higher modes have lower $u_g$ $\Rightarrow$ arrive later. ($u_p$, $\beta$, $f_c$, $Z_\text{TE/TM}$ in Tab.\ 8-3.)}
\end{ebox}
