\documentclass[6pt,letterpaper]{article}
\usepackage[margin=0.25in, top=0.3in, bottom=0.25in]{geometry}
\usepackage{amsmath,amssymb}
\usepackage{multicol}
\usepackage{xcolor}
\usepackage{mdframed}
\usepackage[expansion=false]{microtype}
\usepackage{fancyhdr}
\usepackage{booktabs}
\usepackage{colortbl}
\usepackage{array}
\usepackage{graphicx}

% Colors (match Exam1 / HW3 sheets)
\definecolor{purple}{HTML}{534AB7}\definecolor{purplebg}{HTML}{EEEDFE}
\definecolor{teal}{HTML}{0F6E56}\definecolor{tealbg}{HTML}{E1F5EE}
\definecolor{coral}{HTML}{993C1D}\definecolor{coralbg}{HTML}{FAECE7}
\definecolor{amber}{HTML}{854F0B}\definecolor{amberbg}{HTML}{FAEEDA}
\definecolor{green}{HTML}{3B6D11}\definecolor{greenbg}{HTML}{EAF3DE}
\definecolor{blue}{HTML}{0C447C}\definecolor{bluebg}{HTML}{E6F1FB}
\definecolor{gray}{HTML}{444441}\definecolor{graybg}{HTML}{F1EFE8}
\definecolor{pink}{HTML}{72243E}\definecolor{pinkbg}{HTML}{FBEAF0}
\definecolor{rowB}{HTML}{F2F2EF}

\newmdenv[linewidth=0.4pt,innerleftmargin=3pt,innerrightmargin=3pt,
  innertopmargin=0.5pt,innerbottommargin=0.5pt,skipabove=0.5pt,skipbelow=0.5pt]{ebox}

\newcommand{\shead}[3]{%
  \noindent\colorbox{#1}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}%
  {\color{#2}\bfseries\fontsize{6}{7}\selectfont #3}}\vspace{0.3pt}}

\newcommand{\eq}[2]{\noindent{\color{gray}\bfseries\fontsize{5.8}{6}\selectfont#1}\enspace{\color{teal}\fontsize{5.5}{6}\selectfont\textit{#2}}}

\setlength{\parindent}{0pt}\setlength{\parskip}{0pt}
\setlength{\columnsep}{5pt}\setlength{\multicolsep}{1pt}

\pagestyle{fancy}\fancyhf{}
\renewcommand{\headrulewidth}{0.3pt}
\fancyhead[L]{\fontsize{6.5}{7}\selectfont\textbf{ECE332 --- HW4 Cheat Sheet (Antennas + Friis)}}
\fancyhead[R]{\fontsize{6.5}{7}\selectfont Bao Nguyen}

\graphicspath{{../img/}{../../Cheatsheets/Exam1/img/}{../../../Textbook/tables/}}

\begin{document}

\fontsize{6}{7}\selectfont
\setlength{\abovedisplayskip}{0pt}\setlength{\belowdisplayskip}{0.5pt}
\setlength{\abovedisplayshortskip}{0pt}\setlength{\belowdisplayshortskip}{0pt}
\begin{multicols}{3}

%-- Column 1 -----------------------------------
\shead{purplebg}{purple}{DIPOLE TYPE BY LENGTH}
\begin{ebox}
\eq{Compute first}{$\lambda\!=\!c/f$, then ratio $l/\lambda$}\\[1pt]
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.22\linewidth}p{0.32\linewidth}p{0.38\linewidth}@{}}
\toprule
$l/\lambda$ & Type & Use formula\\
\midrule
\rowcolor{rowB}$<1/50$ & Hertzian (short) & HERTZIAN $S(R,\theta)$, $D\!=\!1.5$\\
$=1/4$ & Quarter-wave & ARB formula, $\pi l/\lambda\!=\!\pi/4$\\
\rowcolor{rowB}$=1/2$ & Half-wave & half-wave shortcut, $D\!=\!1.64$, $R_{\text{rad}}\!=\!73\,\Omega$\\
$=1$ & Full-wave & ARB, $D\!=\!2.41$, $R_{\text{rad}}\!=\!199\,\Omega$\\
\rowcolor{rowB}other & Arbitrary & ARB formula\\
\bottomrule
\end{tabular}}}\par}
\textit{\color{coral}If $l/\lambda<1/50$ STOP and use Hertzian. Do NOT plug small $l$ into the arbitrary formula --- wrong shape.}
\end{ebox}

\vspace{1pt}
\shead{purplebg}{purple}{HERTZIAN DIPOLE --- $S(R,\theta)$}
\begin{ebox}
\eq{Far-field E,H phasors}{small $l\!\ll\!\lambda$}
\[\tilde{E}_\theta = \tfrac{j I_0 k l \eta_0}{4\pi}\tfrac{e^{-jkR}}{R}\sin\theta,\;\; \tilde{H}_\phi = \tilde{E}_\theta/\eta_0\]
\eq{Power density}{$l/\lambda<1/50$; far field}
\[S(R,\theta) = \tfrac{\eta_0 k^2 I_0^2 l^2}{32\pi^2 R^2}\sin^2\theta = S_{\max}\sin^2\theta\,(\text{W/m}^2)\]
$S(R,\theta)$ --- avg.\ pwr dens.\ at $(R,\theta)$ (W/m$^2$)\\
$l$ --- antenna length (m, the rod itself)\\
$I_0$ --- peak current in dipole (A)\\
$R$ --- distance from antenna to obs.\ point (m)\\
$\theta$ --- angle from dipole axis (rad/$^\circ$)\\
$k\!=\!2\pi/\lambda$ --- wavenumber (rad/m)\\
$\eta_0\!=\!120\pi\!\approx\!377\,\Omega$ --- free-space impedance\\[1pt]
\eq{Max direction}{broadside}\ \ $\theta_{\max}\!=\!\pi/2$\\
\eq{Total radiated power}{integrate over $4\pi$}
\[P_{\text{rad}} = \tfrac{\eta_0 \pi}{3}\!\left(\tfrac{I_0 l}{\lambda}\right)^{\!2}\,(\text{W})\]
\eq{Directivity}{exact}\ \ $D_{\text{Hertz}}\!=\!1.5$ ($1.76$ dB)
\end{ebox}

\vspace{1pt}
\shead{tealbg}{teal}{ARBITRARY-LENGTH DIPOLE --- $S(\theta)$}
\begin{ebox}
\eq{Power density at $R$}{any $l$}
\[S(\theta) = \tfrac{15 I_0^2}{\pi R^2}\!\left[\tfrac{\cos\!\left(\tfrac{\pi l}{\lambda}\cos\theta\right)-\cos\!\left(\tfrac{\pi l}{\lambda}\right)}{\sin\theta}\right]^{\!2}\]
\textit{\color{coral}At $\theta\!=\!0,\pi$: bracket is $0/0$. L'H\^opital $\Rightarrow$ $S\!=\!0$ (no axial radiation). TI-36X Pro alt: evaluate bracket at $\theta\!=\!0.001$ rad $\to$ tiny $\to 0$.}\\[1pt]
\eq{Normalized pattern}{dimensionless}\ \ $F(\theta)\!=\!S(\theta)/S_{\max}$\\[1pt]

\eq{Quarter-wave $l\!=\!\lambda/4$}{$\pi l/\lambda\!=\!\pi/4$, $\cos(\pi/4)\!=\!\sqrt{2}/2$}
\[\tfrac{S(\theta)}{S_0}\!=\!\left[\tfrac{\cos(\tfrac{\pi}{4}\cos\theta)-\tfrac{\sqrt{2}}{2}}{\sin\theta}\right]^{\!2},\;S_0\!=\!\tfrac{15I_0^2}{\pi R^2}\]
\[S_{\max} = S(\pi/2) = S_0\!\left(1-\tfrac{\sqrt{2}}{2}\right)^{\!2}\!\approx 0.0858\,S_0\]

\eq{Half-wave $l\!=\!\lambda/2$}{$\pi l/\lambda\!=\!\pi/2$, shortcut form}
\[S(\theta) = \tfrac{15 I_0^2}{\pi R^2}\tfrac{\cos^2(\tfrac{\pi}{2}\cos\theta)}{\sin^2\theta}\]
$D\!=\!1.64$, $R_{\text{rad}}\!=\!73\,\Omega$, $\theta_{\max}\!=\!\pi/2$.
\end{ebox}

\vspace{1pt}
\shead{graybg}{gray}{COMMON ANGLES (deg $\leftrightarrow$ rad, trig values)}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}cccccc@{}}
\toprule
$^\circ$ & rad & $\sin\theta$ & $\cos\theta$ & $\sin^2\theta$ & $\cos^2\theta$\\
\midrule
\rowcolor{rowB}0   & 0       & 0          & 1          & 0   & 1\\
30  & $\pi/6$ & $1/2$      & $\sqrt{3}/2$ & $1/4$ & $3/4$\\
\rowcolor{rowB}45  & $\pi/4$ & $\sqrt{2}/2$ & $\sqrt{2}/2$ & $1/2$ & $1/2$\\
60  & $\pi/3$ & $\sqrt{3}/2$ & $1/2$      & $3/4$ & $1/4$\\
\rowcolor{rowB}90  & $\pi/2$ & 1          & 0          & 1   & 0\\
180 & $\pi$   & 0          & $-1$       & 0   & 1\\
\bottomrule
\end{tabular}}}\par}
\textit{Convert: $\theta_\text{rad}\!=\!\theta_\text{deg}\cdot\pi/180$. Antenna formulas usually take $\theta$ in rad.}
\end{ebox}

\columnbreak

%-- Column 2 -----------------------------------
\shead{greenbg}{green}{BEAM PARAMETERS --- $\beta$, $\Omega_p$, $D$}
\begin{ebox}
\eq{Step 1 --- Normalize}{always start here}
\[F(\theta) = S(\theta)/S_{\max}\;(\text{unitless})\]

\eq{Step 2 --- HPBW $\beta$}{half-power beamwidth, full angular width at $-3$ dB}
\[\text{Set }F(\theta)\!=\!0.5,\; \text{solve for }\theta\;\to\;\theta_{1/2}\]
\[\beta = 2\theta_{1/2}\;(\text{symmetric beams, rad or }^\circ)\]
\textit{\color{coral}Gaussian:}
\[\color{coral}F(\theta_{1/2})\!=\!e^{-a\theta_{1/2}^2}\!=\!0.5\;\Rightarrow\;\theta_{1/2}\!=\!\sqrt{\ln 2/a}\]

\eq{Step 3 --- Pattern solid angle $\Omega_p$}{units: sr}
\[\Omega_p = \!\int_0^{2\pi}\!\!\int_0^{\pi}\!F(\theta)\sin\theta\,d\theta\,d\phi\]
\textit{No $\phi$ dep.: $\Omega_p\!=\!2\pi\!\int_0^\pi\!F(\theta)\sin\theta\,d\theta$.}\\
\textit{\color{coral}TI-36X Pro: \texttt{[2nd][d/dx]} for $\int_\square^\square$, set RAD mode, multiply result by $2\pi$.}\\
\textit{\color{coral}Narrow-beam Gaussian shortcut ($\sin\theta\!\approx\!\theta$, $\to\!\infty$):}
\[\color{coral}\Omega_p \approx \tfrac{\pi}{a} = \tfrac{\pi\,\theta_{1/2}^2}{\ln 2}\quad(F\!=\!e^{-a\theta^2})\]\\[1pt]

\eq{Step 4 --- Directivity $D$}{}
\[D = \tfrac{4\pi}{\Omega_p}\quad(\text{unitless})\qquad D_{\text{dB}}\!=\!10\log_{10}D\]
\textit{Alt.\ with two orthogonal HPBWs: $D\!\approx\!4\pi/(\beta_{xz}\beta_{yz})$.}\\[1pt]

\eq{Step 5 --- Gain (if asked)}{$\xi\!=\!$ rad.\ eff., $0<\xi\le1$}
\[G = \xi D\qquad G_{\text{dB}}\!=\!10\log_{10}G\]
\textit{Lossless / ideal antenna $\Rightarrow$ $G\!=\!D$.}
\end{ebox}

\vspace{1pt}
\shead{amberbg}{amber}{COMMON DIPOLE PARAMETERS}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{1.5pt}\begin{tabular}{@{}p{0.20\linewidth}p{0.18\linewidth}p{0.10\linewidth}p{0.12\linewidth}p{0.30\linewidth}@{}}
\toprule
Type & $l$ & $D$ & $D_{\text{dB}}$ & $R_{\text{rad}}$ ($\Omega$)\\
\midrule
\rowcolor{rowB}Isotropic & N/A & 1 & 0 & ---\\
Hertzian & $<\lambda/50$ & 1.5 & 1.76 & $80\pi^2(l/\lambda)^2$\\
\rowcolor{rowB}Half-wave & $\lambda/2$ & 1.64 & 2.15 & $73.2$\\
$\lambda/4$ monopole & $\lambda/4$ (gnd) & 1.64 & 2.15 & $36.6$\\
\rowcolor{rowB}Full-wave & $\lambda$ & 2.41 & 3.82 & 199\\
\bottomrule
\end{tabular}}}\par}
\textit{Half-wave: $P_{\text{rad}}\!=\!36.6 I_0^2$ W. $\lambda/4$ monopole (above ground): same patt.\ as half-wave but $P_{\text{rad}}, R_{\text{rad}}$ halved.}\\
\textit{\color{coral}Lossless antenna ($\xi\!=\!1$) $\Rightarrow$ $G\!=\!D$. So in Friis, use $G_t,G_r$ = these $D$ values (e.g.\ half-wave $G\!=\!1.64$).}
\end{ebox}

\vspace{1pt}
\shead{amberbg}{amber}{EFFECTIVE AREA $A_e$}
\begin{ebox}
\eq{Definition}{antenna's RX cross section (m$^2$)}
\[A_e = \tfrac{\lambda^2 D}{4\pi}\;(\text{m}^2)\]
\eq{Physical cross section}{wire dipole, diameter $d$}
\[A_p = l\,d = \tfrac{\lambda}{2}\,d\;\text{(half-wave)}\]
\eq{Inverse: $D$ from $A_e$}{}
\[D = \tfrac{4\pi A_e}{\lambda^2}\;(\text{unitless})\]
\eq{Hertzian special case}{$D\!=\!1.5$}
\[A_e^{\text{Hertz}} = \tfrac{3\lambda^2}{8\pi} \approx 0.119\lambda^2\;(\text{m}^2)\]
\textit{\color{coral}Thin wire: $A_e\!\gg\!A_p$. Antenna captures field over $\sim\!\lambda$.}
\end{ebox}

\vspace{1pt}
\shead{coralbg}{coral}{$R_{\text{rad}}$, EFFICIENCY, POWER SPLIT}
\begin{ebox}
\eq{Radiation resistance}{$\Omega$, equiv.\ ckt.\ load absorbing $P_{\text{rad}}$}
\[R_{\text{rad}} = \tfrac{2 P_{\text{rad}}}{I_0^2}\quad R_{\text{loss}} = \tfrac{2 P_{\text{loss}}}{I_0^2}\]
\eq{Input power split}{$P_t$ from generator}
\[P_t = P_{\text{rad}} + P_{\text{loss}}\quad R_{\text{in}} = R_{\text{rad}} + R_{\text{loss}}\]
\eq{Radiation efficiency}{two equivalent forms}
\[\xi = \tfrac{P_{\text{rad}}}{P_t} = \tfrac{R_{\text{rad}}}{R_{\text{rad}}+R_{\text{loss}}}\;(\text{unitless})\]
\eq{Receiving antenna}{$\tilde{V}_{\text{oc}}\!=\!\tilde{E}^i l$ for short dipole}\\
Max pwr transfer when $Z_L\!=\!Z_{\text{in}}^*$ (conjugate match):
\[P_L = P_{\text{int}} = \tfrac{|\tilde{V}_{\text{oc}}|^2}{8R_{\text{rad}}}\;(\text{W})\]
\[A_e = \tfrac{30\pi|\tilde{V}_{\text{oc}}|^2}{R_{\text{rad}}|\tilde{E}^i|^2}\;(\text{m}^2,\;\text{alt.\ form})\]
\end{ebox}

\columnbreak

%-- Column 3 -----------------------------------
\shead{pinkbg}{pink}{FRIIS TRANSMISSION FORMULA}
\begin{ebox}
\eq{Power density at RX antenna}{TX gain $G_t$, range $R$}
\[S_r = \tfrac{G_t\,P_t}{4\pi R^2}\;(\text{W/m}^2)\]
\textit{Energy spreads over sphere of area $4\pi R^2$, with TX gain $G_t$ concentrating it.}\\[1pt]
\eq{Far-field link budget}{$P_t$ tx, $P_r$ rx, range $R$}
\[\boxed{P_r = G_t G_r\!\left(\tfrac{\lambda}{4\pi R}\right)^{\!2}\!P_t}\;(\text{W})\]
\textit{$G_t, G_r$ are \emph{linear} gains;\, $\lambda\!=\!c/f$;\, $R$ in m;\, $P_t, P_r$ in W. Equivalent: $P_r = S_r A_{e,r}$.}\\[1pt]
\eq{Solve for $R$}{max range}
\[R = \tfrac{\lambda}{4\pi}\sqrt{\tfrac{G_tG_rP_t}{P_r}}\;(\text{m})\]
\eq{Equivalent form using $A_e$}{recv.\ area form}
\[P_r = \tfrac{G_t P_t A_{e,r}}{4\pi R^2},\quad G_r = \tfrac{4\pi A_{e,r}}{\lambda^2}\]
\textit{Free-space path loss: $L_{\text{fs}}\!=\!(4\pi R/\lambda)^2$, so $P_r\!=\!G_tG_rP_t/L_{\text{fs}}$.}\\[1pt]
\eq{General form (off-axis)}{when patterns NOT peak-aligned}
\[P_r = G_tG_r\!\left(\tfrac{\lambda}{4\pi R}\right)^{\!2}\!P_t\,F_t(\theta_t,\phi_t)F_r(\theta_r,\phi_r)\]
\textit{$F_t, F_r$ normalized patterns at the link directions; $F\!=\!1$ when aligned.}\\[1pt]
\eq{Recipe}{}
1.\ $\lambda\!=\!c/f$.\ \ 2.\ Convert dB gains to linear: $G\!=\!10^{G_{\text{dB}}/10}$.\\
3.\ If TX is ``half-wave dipole'' use $G_t\!=\!1.64$.\ \ 4.\ Plug into Friis.\\[1pt]
\textit{\color{coral}Use \emph{linear} $G$, not dB. Convert FIRST.}
\end{ebox}

\vspace{1pt}
\shead{pinkbg}{pink}{dB $\leftrightarrow$ LINEAR}
\begin{ebox}
\[G_{\text{dB}} = 10\log_{10}G_{\text{lin}}\qquad G_{\text{lin}} = 10^{G_{\text{dB}}/10}\]
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}cc@{\quad}cc@{}}
\toprule
dB & linear & dB & linear\\
\midrule
\rowcolor{rowB}0 & 1 & 10 & 10\\
3 & 2 & 13 & 20\\
\rowcolor{rowB}6 & 4 & 20 & 100\\
7 & 5 & 30 & 1000\\
\rowcolor{rowB}$-3$ & $0.5$ & $-10$ & $0.1$\\
\bottomrule
\end{tabular}}}\par}
\textit{Memorize: 3 dB $\!\Leftrightarrow\!\times2$, 10 dB $\!\Leftrightarrow\!\times10$. Add dB $=$ multiply linear.}\\
\textit{\color{coral}For E-field / amplitude use $20\log$ not $10\log$ (power $\propto$ E$^2$).}
\end{ebox}

\vspace{1pt}
\shead{greenbg}{green}{POWER FLOW IDENTITIES}
\begin{ebox}
\[P_{\text{rad}} = S_{\max}\,\Omega_p\,R^2 = \tfrac{4\pi R^2 S_{\max}}{D}\,(\text{W})\]
\[S_{\max} = \tfrac{P_{\text{rad}}\,D}{4\pi R^2}\,(\text{W/m}^2)\]
\textit{Isotropic radiator: $S\!=\!P_{\text{rad}}/(4\pi R^2)$ (no $D$ factor).}
\end{ebox}

\vspace{1pt}
\shead{bluebg}{blue}{NOISE \& SNR}
\begin{ebox}
\eq{Noise power}{thermal noise into matched receiver}
\[P_N = k_B\,T_{\text{sys}}\,B\;(\text{W})\]
$k_B\!=\!1.38{\times}10^{-23}$ J/K; $T_{\text{sys}}$ system noise temp (K); $B$ receiver bandwidth (Hz).\\[1pt]
\eq{Signal-to-noise ratio}{}
\[\text{SNR} = \tfrac{P_{\text{rec}}}{P_N}\;(\text{unitless})\quad\text{SNR}_{\text{dB}}\!=\!10\log_{10}\tfrac{P_{\text{rec}}}{P_N}\]
\textit{Typical comm.\ link needs SNR $>10$ dB for usable signal.}
\end{ebox}

\vspace{1pt}
\shead{tealbg}{teal}{APERTURE ANTENNAS (horn, dish)}
\begin{ebox}
\eq{Directivity (any shape, uniform illum.)}{}
\[D \approx \tfrac{4\pi A_p}{\lambda^2}\;(\text{unitless})\quad\Rightarrow\;A_e\!\approx\!A_p\]
\eq{Directivity from beamwidths}{any aperture}
\[D \approx \tfrac{4\pi}{\beta_{xz}\beta_{yz}}\;\text{(use rad);\ circular: }D\!\approx\!4\pi/\beta^2\]\\[1pt]

\eq{HPBW by aperture shape}{uniform illum.; $\lambda$ same units as size}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.20\linewidth}p{0.32\linewidth}p{0.40\linewidth}@{}}
\toprule
Shape & $A_p$ & HPBW (rad)\\
\midrule
\rowcolor{rowB}Rect.\ $\ell_x\!\times\!\ell_y$ & $\ell_x\ell_y$ & $\beta_x\!\approx\!0.88\lambda/\ell_x,\;\beta_y\!\approx\!0.88\lambda/\ell_y$\\
Square $\ell$ & $\ell^2$ & $\beta\!\approx\!0.88\lambda/\ell$\\
\rowcolor{rowB}Circular (diam.\ $d$) & $\pi d^2/4$ & $\beta\!\approx\!1.02\lambda/d \approx 58.4^\circ(\lambda/d)$\\
\bottomrule
\end{tabular}}}\par}
\textit{\color{coral}Scaling: $D\propto A_p/\lambda^2$; $\beta\propto\lambda/\sqrt{A_p}$. Double area $\Rightarrow$ $D{\times}2$, $\beta/\sqrt{2}$. Double $f$ ($\lambda/2$) $\Rightarrow$ $D{\times}4$, $\beta/2$.}\\
\textit{Tapered illumination widens main lobe ($\sim 1.2$--$1.5\times$ wider), reduces side lobes.}
\end{ebox}

\vspace{1pt}
\shead{pinkbg}{pink}{LINEAR ANTENNA ARRAY}
\begin{ebox}
\eq{Setup}{$N$ identical elements, spacing $d$ along axis}
\[\text{element pos } z_i = id,\;i = 0,\ldots,N\!-\!1;\;A_i = a_i e^{j\psi_i}\]
\eq{Pattern multiplication}{}
\[S(R,\theta,\phi) = S_e(R,\theta,\phi)\,F_a(\theta)\]
\eq{Array factor}{$F_a(\theta) = |\sum A_i e^{jkid\cos\theta}|^2$}
\[F_a(\theta) = \left|\sum_{i=0}^{N-1} A_i\,e^{jkid\cos\theta}\right|^2\]
\eq{Uniform $A_i\!=\!1$, equal phase}{closed form}
\[F_a(\theta) = \tfrac{\sin^2(N\gamma/2)}{\sin^2(\gamma/2)},\;\gamma\!=\!kd\cos\theta\]
\textit{Normalize: $F_a/N^2$. Peak $N^2$ at $\gamma\!=\!0$ ($\cos\theta\!=\!0$, broadside).}\\[1pt]
\eq{HPBW (broadside, uniform)}{array length $L\!=\!(N{-}1)d$}
\[\beta \approx 0.88\,\tfrac{\lambda}{L}\;(\text{rad})\]
\end{ebox}

\vspace{1pt}
\shead{graybg}{gray}{SYMBOL REFERENCE}
\begin{ebox}
$l$ --- antenna length (m); $\lambda\!=\!c/f$ (m); $c\!=\!3\!\times\!10^8$ m/s\\
$I_0$ --- peak current (A); $\eta_0\!=\!120\pi\!\approx\!377\,\Omega$\\
$k\!=\!2\pi/\lambda$ wavenumber (rad/m); $R$ obs.\ dist (m)\\
$S(R,\theta)$ --- power density (W/m$^2$)\\
$F(\theta)\!=\!S/S_{\max}$ --- normalized pattern (unitless)\\
$\beta$ --- half-power beamwidth (rad or deg)\\
$\Omega_p$ --- pattern solid angle (sr)\\
$D\!=\!4\pi/\Omega_p$ --- directivity (unitless); $\xi$ rad.\ eff.; $G\!=\!\xi D$\\
$A_e\!=\!\lambda^2 D/(4\pi)$ --- effective area (m$^2$); $A_p\!=\!l d$ phys.\ (m$^2$)\\
$R_{\text{rad}}$ --- radiation resistance ($\Omega$); $R_{\text{loss}}$ loss res.\ ($\Omega$)\\
$P_t, P_r$ --- TX / RX power (W); $G_t,G_r$ linear gains
\end{ebox}

\end{multicols}

\end{document}
