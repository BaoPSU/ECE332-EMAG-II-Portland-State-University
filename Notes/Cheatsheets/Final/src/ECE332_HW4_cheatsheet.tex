...
\textit{Off-axis (patterns not aligned): multiply by $F_t(\theta_t,\phi_t)F_r(\theta_r,\phi_r)$, both $=1$ when aligned.}\\[1pt]
\eq{Recipe}{}
1.\ $\lambda\!=\!c/f$.\ 2.\ Convert any dB gain $\to$ linear: $G\!=\!10^{G_{\text{dB}}/10}$.\\
3.\ Get $G_t,G_r$ by antenna type: dipole$\to$COMMON DIPOLE PARAMS; aperture$\to$APERTURE box; arbitrary $F\to$BEAM PARAMS.\ 4.\ Plug into Friis.\\
