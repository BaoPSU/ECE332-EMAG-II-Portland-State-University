...
\[V_\text{emf} = -\frac{d\Phi}{dt}\;\text{(V)} \qquad \Phi = \int_S \mathbf{B} \cdot d\mathbf{s}\;\text{(Wb)}\]

\eq{$N$-turn coil}{transformer, $N$ turns}
\[V_\text{emf} = -N\frac{d\Phi}{dt} = -\frac{d\lambda}{dt}\;\text{(V)} \quad \lambda=N\Phi\;\text{(Wb)}\]
