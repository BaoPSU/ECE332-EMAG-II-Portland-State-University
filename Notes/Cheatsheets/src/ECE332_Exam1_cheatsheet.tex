...
\rowcolor{rowB}$\mathbf{E}(z,t)$ & $E_0\cos(\omega t-kz+\phi)\,\hat{e}$ & \\
$\mathbf{H}(z,t)$ & $\tfrac{E_0}{\eta}\cos(\omega t-kz+\phi)\,(\hat{k}\times\hat{e})$ & \\
\rowcolor{rowB}$\mathbf{H}(t)$ direct & $\tfrac{1}{\eta}(\hat{k}\times\mathbf{E}(t))$ & \\
...
\textit{\color{gray}$\delta=0$: $(a_x\hat{x}+a_y\hat{y})\cos(\omega t\!-\!kz)$;\enspace$\delta=\pi$: $(a_x\hat{x}-a_y\hat{y})\cos(\omega t\!-\!kz)$\enspace\textbf{(linear)}}\\
...
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.47\linewidth}p{0.48\linewidth}@{}}
\toprule
Phasor coeff & $\operatorname{Re}\{(\cdot)\,e^{j\omega t}e^{-\alpha z}e^{-j\beta z}\}$\\
\midrule
\rowcolor{rowB}$A$\enspace$(=\!Ae^{j\cdot0})$ & $A\cos(\omega t-\beta z)$\\
$-jA$\enspace$(=\!Ae^{-j\pi/2})$ & $A\sin(\omega t-\beta z)$\\
\rowcolor{rowB}$+jA$\enspace$(=\!Ae^{+j\pi/2})$ & $-A\sin(\omega t-\beta z)$\\
$-A$\enspace$(=\!Ae^{\pm j\pi})$ & $-A\cos(\omega t-\beta z)$\\
\rowcolor{rowB}$Ae^{j\phi}$ (general) & $A\cos(\omega t-\beta z+\phi)$\\
$A+jB$\enspace$(=\!|Z|e^{j\theta})$ & $|Z|\cos(\omega t-\beta z+\theta)$\\
\bottomrule
\end{tabular}}}\par}\vspace{1pt}
