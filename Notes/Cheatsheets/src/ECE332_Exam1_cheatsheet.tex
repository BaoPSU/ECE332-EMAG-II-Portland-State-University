...
$R'\!=\!|\mathbf{R}\!-\!\mathbf{R}_i|$ (m);\enspace $t\!-\!R'/u_p$ retarded time;\enspace $u_p\!\approx\! c$\\
$\rho_v$: C/m$^3$;\enspace $\mathbf{J}$: A/m$^2$;\enspace $V$(V) scalar;\enspace $\mathbf{A}$(Wb/m) vec pot
\end{ebox}
...
\begin{ebox}
$G$: power gain (dB)\\
$P_1,P_2$: power levels (W)\\
$|E_1|,|E_2|$: field magnitudes (V/m)\\
$A$: attenuation over $z$ (dB)\\
$\alpha$: atten.\ const (Np/m);\; $z$: dist (m)\\
20 log not 10: power $\propto E^2$\\
Np/m $\times$ 8.69 = dB/m
\end{ebox}
\end{minipage}
