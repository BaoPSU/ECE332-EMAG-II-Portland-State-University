\documentclass[6pt,letterpaper]{article}
\usepackage[margin=0.25in, top=0.3in, bottom=0.25in]{geometry}
\usepackage{amsmath,amssymb}
\usepackage{multicol}
\usepackage{xcolor}
\usepackage{mdframed}
\usepackage[expansion=false]{microtype}
\usepackage{fancyhdr}
\usepackage{booktabs}
\usepackage{colortbl}
\usepackage{array}
\usepackage{graphicx}

\definecolor{purple}{HTML}{534AB7}\definecolor{purplebg}{HTML}{EEEDFE}
\definecolor{teal}{HTML}{0F6E56}\definecolor{tealbg}{HTML}{E1F5EE}
\definecolor{coral}{HTML}{993C1D}\definecolor{coralbg}{HTML}{FAECE7}
\definecolor{amber}{HTML}{854F0B}\definecolor{amberbg}{HTML}{FAEEDA}
\definecolor{green}{HTML}{3B6D11}\definecolor{greenbg}{HTML}{EAF3DE}
\definecolor{blue}{HTML}{0C447C}\definecolor{bluebg}{HTML}{E6F1FB}
\definecolor{gray}{HTML}{444441}\definecolor{graybg}{HTML}{F1EFE8}
\definecolor{pink}{HTML}{72243E}\definecolor{pinkbg}{HTML}{FBEAF0}
\definecolor{rowB}{HTML}{F2F2EF}

\newmdenv[linewidth=0.4pt,innerleftmargin=3pt,innerrightmargin=3pt,
  innertopmargin=0.5pt,innerbottommargin=0.5pt,skipabove=0.5pt,skipbelow=0.5pt]{ebox}

\newcommand{\shead}[3]{%
  \noindent\colorbox{#1}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}%
  {\color{#2}\bfseries\fontsize{6}{7}\selectfont #3}}\vspace{0.5pt}}

\newcommand{\eq}[2]{\noindent{\color{gray}\bfseries\fontsize{5.8}{6}\selectfont#1}\enspace{\color{teal}\fontsize{5.5}{6}\selectfont\textit{#2}}}

\setlength{\parindent}{0pt}\setlength{\parskip}{0pt}
\setlength{\abovedisplayskip}{0pt}\setlength{\belowdisplayskip}{0.5pt}
\setlength{\abovedisplayshortskip}{0pt}\setlength{\belowdisplayshortskip}{0pt}
\setlength{\columnsep}{5pt}\setlength{\multicolsep}{1pt}

\pagestyle{fancy}\fancyhf{}
\renewcommand{\headrulewidth}{0.3pt}
\fancyhead[L]{\fontsize{6.5}{7}\selectfont\textbf{ECE332 --- Exam 1 Cheat Sheet (HW1 + HW2)}}
\fancyhead[R]{\fontsize{6.5}{7}\selectfont Bao Nguyen}

\begin{document}
\fontsize{6}{7}\selectfont

%%=============================================================
%% PAGE 1 — HW1 CONTENT
%%=============================================================
\begin{multicols}{3}

%────── COL 1 ──────
\shead{purplebg}{purple}{EMF \& FARADAY'S LAW}
\begin{ebox}
\eq{Transformer EMF}{stationary loop, $B$ changes}
\[V_\text{emf}=-\int_S\frac{\partial\mathbf{B}}{\partial t}\cdot d\mathbf{s}\]
\eq{Motional EMF}{loop moves, $B$ static}
\[V_\text{emf}=\oint(\mathbf{v}\times\mathbf{B})\cdot d\mathbf{l}\]
\eq{General (flux form)}{always works}
\[V_\text{emf}=-\frac{d\Phi}{dt}\qquad\Phi=\int_S\mathbf{B}\cdot d\mathbf{s}\]
\eq{$N$-turn coil}{transformer, $N$ turns}
\[V_\text{emf}=-N\frac{d\Phi}{dt}=-\frac{d\lambda}{dt}\quad\lambda=N\Phi\]
\eq{Uniform $B$ shortcut}{$B$ same everywhere on loop}
\[V_\text{emf}=-\frac{\partial B}{\partial t}\cdot A\]
\eq{Long wire + rect loop}{wire near loop, $r$: $a\to b$, height $c$}
\[V_\text{emf}=-\int_0^c\!\int_a^b\frac{\partial}{\partial t}\!\left(\frac{\mu_0 I}{2\pi r}\right)dr\,dz\]
\end{ebox}

\vspace{0.3pt}
\shead{purplebg}{purple}{SURFACE ELEMENT $d\mathbf{s}$}
\begin{ebox}
$d\mathbf{s}=\hat{n}\,da$\quad\textit{RHR: curl fingers in loop dir., thumb $=\hat{n}$}\\[1pt]
{\centering\makebox[0.98\linewidth][c]{\begin{tabular}{@{}ll@{}}
\toprule
Loop plane & $d\mathbf{s}$\\
\midrule
\rowcolor{rowB}$x$-$y$ & $\hat{z}\,dx\,dy$ or $\hat{z}\,r\,dr\,d\phi$\\
$y$-$z$ & $\hat{x}\,dy\,dz$\\
\rowcolor{rowB}$x$-$z$ & $\hat{y}\,dx\,dz$\\
coplanar/toroid & $\hat{\phi}\,dr\,dz$\\
\bottomrule
\end{tabular}}\par}
\textit{Uniform $B$: $\iint da=A$ (just the loop area)}
\end{ebox}

\vspace{0.3pt}
\shead{purplebg}{purple}{DOT PRODUCTS $\hat{a}\cdot\hat{b}$}
\begin{ebox}
Same dir.$=1$;\quad perpendicular$=0$\\[1pt]
{\centering\makebox[0.98\linewidth][c]{\begin{tabular}{@{}cccccc@{}}
\toprule
& $\hat{x}$ & $\hat{y}$ & $\hat{z}$ & $\hat{r}$ & $\hat{\phi}$\\
\midrule
\rowcolor{rowB}$\hat{x}$ & 1 & 0 & 0 & 0 & 0\\
$\hat{y}$ & 0 & 1 & 0 & 0 & 0\\
\rowcolor{rowB}$\hat{z}$ & 0 & 0 & 1 & 0 & 0\\
$\hat{\phi}$ & 0 & 0 & 0 & 0 & 1\\
\bottomrule
\end{tabular}}\par}
\end{ebox}

\vspace{0.3pt}
\shead{purplebg}{purple}{CROSS PRODUCT $\mathbf{A}\times\mathbf{B}$}
\begin{ebox}
\[\mathbf{A}\times\mathbf{B}=\begin{vmatrix}\hat{x}&\hat{y}&\hat{z}\\A_x&A_y&A_z\\B_x&B_y&B_z\end{vmatrix}\]
\[=(A_yB_z-A_zB_y)\hat{x}-(A_xB_z-A_zB_x)\hat{y}+(A_xB_y-A_yB_x)\hat{z}\]
\textit{Plane wave ($\partial_x=\partial_y=0$): only $\partial_z$ survives in curl.}
\end{ebox}

\columnbreak

%────── COL 2 ──────
\shead{tealbg}{teal}{$\mathbf{B}$ FIELDS}
\begin{ebox}
\eq{Infinite wire}{single wire, $N=1$}
\[\mathbf{B}=\hat{\phi}\,\frac{\mu_0 I}{2\pi r}\]
\eq{Inside toroid}{$N$-turn coil}
\[\mathbf{B}=\hat{\phi}\,\frac{\mu N I}{2\pi r}\qquad\mu=\mu_r\mu_0\]
\textit{\color{coral}$N$ in $\mathbf{B}$ only if the \textbf{source} is multi-turn. In $V_\text{emf}=-Nd\Phi/dt$, $N$ is the \textbf{receiver} turns.}
\end{ebox}

\vspace{0.3pt}
\shead{tealbg}{teal}{$\partial B/\partial t$ TABLE}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.46\linewidth}p{0.46\linewidth}@{}}
\toprule
$B(t)$ & $\partial B/\partial t$\\
\midrule
\rowcolor{rowB}$B_0\sin(\omega t)$ & $B_0\omega\cos(\omega t)$\\
$B_0\cos(\omega t)$ & $-B_0\omega\sin(\omega t)$\\
\rowcolor{rowB}$B_0 e^{-\alpha t}$ & $-\alpha B_0 e^{-\alpha t}$\\
$B_0$ (const) & $0$\\
\bottomrule
\end{tabular}}}\par}
\end{ebox}

\vspace{0.3pt}
\shead{tealbg}{teal}{SIGN $\to$ CURRENT DIRECTION}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.26\linewidth}p{0.34\linewidth}p{0.28\linewidth}@{}}
\toprule
$V_\text{emf}$ & Inside source & $I$ dir.\\
\midrule
\rowcolor{rowB}$<0$ & $-\;\to\;+$ terminal & $-\hat{\phi}$ (CW)\\
$>0$ & $+\;\to\;-$ terminal & $+\hat{\phi}$ (CCW)\\
\rowcolor{rowB}$=0$ & --- & none\\
\bottomrule
\end{tabular}}}\par}
\textit{Lenz: induced $I$ opposes the change in $\Phi$.}
\end{ebox}

\vspace{0.3pt}
\shead{tealbg}{teal}{$\ln$ INTEGRAL}
\begin{ebox}
\eq{}{$B\propto1/r$, loop spans $r=a$ to $r=b$}
\[\int_a^b\frac{dr}{r}=\ln\frac{b}{a}=\ln b-\ln a\]
$\ln 2=0.693\quad\ln 3=1.099\quad\ln 1.2=0.182$
\end{ebox}

\vspace{0.3pt}
\shead{amberbg}{amber}{ROTATING LOOP --- GENERATOR}
\begin{ebox}
\[\Phi=AB_0\cos(\omega t+\phi_0)\]
\[V_\text{emf}=AB_0\omega\sin(\omega t+\phi_0)\]
\[I_\text{amp}=\frac{AB_0\omega}{R}\qquad\omega=2\pi f=2\pi\frac{\text{rpm}}{60}\]
\end{ebox}

\vspace{0.3pt}
\shead{graybg}{gray}{SI PREFIXES}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{\begin{tabular}{@{}lll@{\enspace}lll@{}}
\toprule
Prefix & Sym & Val & Prefix & Sym & Val\\
\midrule
\rowcolor{rowB}pico & p & $10^{-12}$ & kilo & k & $10^3$\\
nano & n & $10^{-9}$ & mega & M & $10^6$\\
\rowcolor{rowB}micro & $\mu$ & $10^{-6}$ & giga & G & $10^9$\\
milli & m & $10^{-3}$ & tera & T & $10^{12}$\\
\bottomrule
\end{tabular}}\par}
\end{ebox}

\vspace{0.3pt}
\shead{pinkbg}{pink}{$j$ IDENTITIES}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{\begin{tabular}{@{}ll@{\enspace}ll@{}}
\toprule
Expr & Result & Expr & Result\\
\midrule
\rowcolor{rowB}$j^2$ & $-1$ & $j\times(-j)$ & $1$\\
$1/j$ & $-j$ & $-1/j$ & $j$\\
\rowcolor{rowB}$j^3$ & $-j$ & $j^4$ & $1$\\
$e^{-j\pi/2}$ & $-j$ & $e^{j\pi/2}$ & $j$\\
\rowcolor{rowB}$e^{j\pi}$ & $-1$ & $e^{j2\pi}$ & $1$\\
\bottomrule
\end{tabular}}\par}
\end{ebox}

\columnbreak

%────── COL 3 ──────
\shead{coralbg}{coral}{DISPLACEMENT \& CONDUCTION CURRENT}
\begin{ebox}
\eq{Conduction}{resistive, $\sigma\ne0$}
\[J_c=\sigma E\qquad I_c=J_c\cdot A\qquad R=\frac{d}{\sigma A}\]
\eq{Displacement}{capacitive, changing $E$}
\[J_d=\varepsilon\frac{\partial E}{\partial t}\qquad I_d=J_d\cdot A\qquad C=\frac{\varepsilon_r\varepsilon_0 A}{d}\]
\eq{Lossy cap circuit}{both $\sigma$ and $\varepsilon_r$}
\[I=I_c+I_d=\frac{V}{R}+C\frac{dV}{dt}\]
\end{ebox}

\vspace{0.3pt}
\shead{coralbg}{coral}{CHARGE RELAXATION}
\begin{ebox}
\[\rho_v(t)=\rho_{v0}\,e^{-t/\tau_r}\qquad\tau_r=\frac{\varepsilon_r\varepsilon_0}{\sigma}\]
\textit{Larger $\tau_r$ = slower dissipation = better insulator.}
\end{ebox}

\vspace{0.3pt}
\shead{pinkbg}{pink}{PHASOR METHOD ($\mathbf{E}\to\mathbf{H}$)}
\begin{ebox}
\textbf{\color{pink}1.} $\mathbf{E}(z,t)=\operatorname{Re}\{\tilde{\mathbf{E}}(z)\,e^{j\omega t}\}$\\
\textbf{\color{pink}2.} Strip $e^{j\omega t}$, keep $e^{-jkz}$:\\
$\cos(\omega t-kz)\to e^{-jkz}$\quad$\sin(\omega t-kz)\to-j\,e^{-jkz}$\\
\textbf{\color{pink}3.} Faraday in phasor (plane wave: only $\partial/\partial z$ survives):
\[\tilde{\mathbf{H}}=-\frac{1}{j\omega\mu}\nabla\times\tilde{\mathbf{E}}\]
\textbf{\color{pink}4.} Back to time: $\operatorname{Re}\{Ae^{j\phi}e^{j(\omega t-kz)}\}=A\cos(\omega t-kz+\phi)$
\end{ebox}

\vspace{0.3pt}
\shead{graybg}{gray}{UNIT CIRCLE}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{\begin{tabular}{@{}lllll@{}}
\toprule
Angle & Deg & $\cos$ & $\sin$ & Q\\
\midrule
\rowcolor{rowB}$0$ & $0°$ & $1$ & $0$ & ---\\
$\pi/6$ & $30°$ & $\sqrt{3}/2$ & $1/2$ & 1\\
\rowcolor{rowB}$\pi/4$ & $45°$ & $\sqrt{2}/2$ & $\sqrt{2}/2$ & 1\\
$\pi/3$ & $60°$ & $1/2$ & $\sqrt{3}/2$ & 1\\
\rowcolor{rowB}$\pi/2$ & $90°$ & $0$ & $1$ & ---\\
$2\pi/3$ & $120°$ & $-1/2$ & $\sqrt{3}/2$ & 2\\
\rowcolor{rowB}$3\pi/4$ & $135°$ & $-\sqrt{2}/2$ & $\sqrt{2}/2$ & 2\\
$5\pi/6$ & $150°$ & $-\sqrt{3}/2$ & $1/2$ & 2\\
\rowcolor{rowB}$\pi$ & $180°$ & $-1$ & $0$ & ---\\
$7\pi/6$ & $210°$ & $-\sqrt{3}/2$ & $-1/2$ & 3\\
\rowcolor{rowB}$5\pi/4$ & $225°$ & $-\sqrt{2}/2$ & $-\sqrt{2}/2$ & 3\\
$4\pi/3$ & $240°$ & $-1/2$ & $-\sqrt{3}/2$ & 3\\
\rowcolor{rowB}$3\pi/2$ & $270°$ & $0$ & $-1$ & ---\\
$5\pi/3$ & $300°$ & $1/2$ & $-\sqrt{3}/2$ & 4\\
\rowcolor{rowB}$7\pi/4$ & $315°$ & $\sqrt{2}/2$ & $-\sqrt{2}/2$ & 4\\
$11\pi/6$ & $330°$ & $\sqrt{3}/2$ & $-1/2$ & 4\\
\rowcolor{rowB}$2\pi$ & $360°$ & $1$ & $0$ & ---\\
\bottomrule
\end{tabular}}\par}
\end{ebox}

\end{multicols}

\vfill
\noindent
\begin{minipage}[t]{0.19\linewidth}
\shead{purplebg}{purple}{FIELDS}
\begin{ebox}
$\mathbf{E}$ — electric field (V/m)\\
$\mathbf{B}$ — mag.\ flux density (T)\\
$\mathbf{H}$ — mag.\ field (A/m)\\
$\rho_v$ — charge density (C/m$^3$)\\
$\Phi=\int_S\mathbf{B}\cdot d\mathbf{s}$ — flux (Wb)\\
$\lambda=N\Phi$ — flux linkage (Wb)
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{tealbg}{teal}{CURRENTS}
\begin{ebox}
$J_c=\sigma E$ — cond.\ density (A/m$^2$)\\
$J_d=\varepsilon\,\partial E/\partial t$ — disp.\ density\\
$I_c=J_c\cdot A$ — cond.\ current (A)\\
$I_d=J_d\cdot A$ — disp.\ current (A)\\
$R=d/(\sigma A)$ — resistance ($\Omega$)\\
$C=\varepsilon_r\varepsilon_0 A/d$ — capacitance (F)\\
$\tau_r=\varepsilon_r\varepsilon_0/\sigma$ — relax.\ time (s)
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{coralbg}{coral}{MATERIALS}
\begin{ebox}
$\mu_0=4\pi\!\times\!10^{-7}$ H/m\\
$\mu_r$ — rel.\ permeability\\
$\mu=\mu_r\mu_0$ — permeability\\[1pt]
$\varepsilon_0\approx\frac{1}{36\pi}\!\times\!10^{-9}$ F/m\\
$\varepsilon_r$ — rel.\ permittivity\\
$\varepsilon=\varepsilon_r\varepsilon_0$ — permittivity\\[1pt]
$\sigma$ — conductivity (S/m)
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{bluebg}{blue}{CIRCUIT \& WAVE}
\begin{ebox}
$V$ — voltage (V)\\
$R$ — resistance ($\Omega$)\\
$C$ — capacitance (F)\\
$\omega=2\pi f$ — ang.\ freq (rad/s)\\
$k=\omega\sqrt{\mu\varepsilon}$ — wavenumber\\
$\lambda=2\pi/k$ — wavelength (m)\\
$c=3\!\times\!10^8$ m/s
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{pinkbg}{pink}{PHASOR \& GEOMETRY}
\begin{ebox}
$\eta=\sqrt{\mu/\varepsilon}$ — imp.\ ($\Omega$)\\
$\eta_0=120\pi\approx377\,\Omega$\\
$k/(\omega\mu)=1/\eta$\\
$j^2=-1$;\enspace$\tilde{\mathbf{E}}$: drop $e^{j\omega t}$\\[1pt]
$A$ — area (m$^2$);\;$d$ — sep.\ (m)\\
$N$ — turns;\;$r$ — radial (m)
\end{ebox}
\end{minipage}

\newpage

%%=============================================================
%% PAGE 2 — HW2 CONTENT
%%=============================================================
\begin{multicols}{3}

%────── COL 1 ──────
\shead{purplebg}{purple}{PLANE WAVE PARAMETERS (LOSSLESS)}
\begin{ebox}
\eq{Angular frequency}{$\omega$: rad/s;\enspace$f$: Hz}
\[\omega=2\pi f\qquad f=\frac{\omega}{2\pi}\]
\eq{Phase velocity}{m/s;\enspace nonmagnetic: $\mu_r=1$}
\[u_p=\frac{c}{\sqrt{\varepsilon_r}}\qquad c=3\times10^8\;\text{m/s}\]
\eq{Wavenumber $k$}{rad/m}
\[k=\frac{\omega}{u_p}=\frac{2\pi}{\lambda}=\frac{\omega\sqrt{\varepsilon_r}}{c}\]
\eq{Wavelength \& impedance}{m \& $\Omega$}
\[\lambda=\frac{u_p}{f}=\frac{2\pi}{k}\;\text{(m)}\qquad\eta=\frac{\eta_0}{\sqrt{\varepsilon_r}}\;(\Omega)\]
\textit{\color{coral}Order: $\omega\!\to\!u_p\!\to\!k\!\to\!\lambda\!\to\!\eta$.}
\end{ebox}

\vspace{0.3pt}
\shead{purplebg}{purple}{LOSSLESS PLANE WAVE --- $\mathbf{E}$ AND $\mathbf{H}$}
\begin{ebox}
\eq{General $\mathbf{E}$}{}
\[\mathbf{E}=E_0\cos(\omega t-k\,\hat{k}\cdot\mathbf{r}+\phi_0)\,\hat{e}\]
\textit{$\hat{k}$: travel dir.;\enspace$\hat{e}$: polarization dir.;\enspace$\hat{k}\cdot\mathbf{r}$ picks the axis.}\\
\eq{$\mathbf{H}$ from $\mathbf{E}$}{$\hat{k}$, $\hat{E}$, $\hat{H}$ form right-hand set}
\[\mathbf{H}=\frac{1}{\eta}\,(\hat{k}\times\mathbf{E})\]
\eq{Phasor form ($+z$ propagation)}{}
\[\tilde{\mathbf{E}}=E_0 e^{j\phi_0}e^{-jkz}\hat{e}\qquad\tilde{\mathbf{H}}=\frac{1}{\eta}\hat{k}\times\tilde{\mathbf{E}}\]
\end{ebox}

\vspace{0.3pt}
\shead{purplebg}{purple}{DIRECTION OF PROPAGATION}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{\begin{tabular}{@{}ll@{\enspace}ll@{}}
\toprule
Arg.\ contains & $\hat{k}$ & Arg.\ contains & $\hat{k}$\\
\midrule
\rowcolor{rowB}$\omega t-kz$ & $+\hat{z}$ & $\omega t+kz$ & $-\hat{z}$\\
$\omega t-ky$ & $+\hat{y}$ & $\omega t+ky$ & $-\hat{y}$\\
\rowcolor{rowB}$\omega t-kx$ & $+\hat{x}$ & $\omega t+kx$ & $-\hat{x}$\\
\bottomrule
\end{tabular}}\par}
\textit{Minus before $k$-axis $\Rightarrow+$ direction. Plus $\Rightarrow-$ direction.}
\end{ebox}

\vspace{0.3pt}
\shead{tealbg}{teal}{$\hat{k}\times\hat{E}$ --- FIND $\hat{H}$ DIRECTION}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{\begin{tabular}{@{}lll@{\enspace}lll@{}}
\toprule
$\hat{k}$ & $\hat{E}$ & $\hat{H}$ & $\hat{k}$ & $\hat{E}$ & $\hat{H}$\\
\midrule
\rowcolor{rowB}$+\hat{z}$ & $\hat{x}$ & $+\hat{y}$ & $+\hat{z}$ & $\hat{y}$ & $-\hat{x}$\\
$+\hat{y}$ & $\hat{x}$ & $-\hat{z}$ & $+\hat{y}$ & $\hat{z}$ & $+\hat{x}$\\
\rowcolor{rowB}$+\hat{x}$ & $\hat{y}$ & $+\hat{z}$ & $+\hat{x}$ & $\hat{z}$ & $-\hat{y}$\\
$-\hat{x}$ & $\hat{y}$ & $-\hat{z}$ & $-\hat{x}$ & $\hat{z}$ & $+\hat{y}$\\
\rowcolor{rowB}$-\hat{y}$ & $\hat{x}$ & $+\hat{z}$ & $-\hat{z}$ & $\hat{x}$ & $-\hat{y}$\\
\bottomrule
\end{tabular}}\par}
\textit{Cyclic: $\hat{x}\!\times\!\hat{y}\!=\!\hat{z}$, $\hat{y}\!\times\!\hat{z}\!=\!\hat{x}$, $\hat{z}\!\times\!\hat{x}\!=\!\hat{y}$. Reverse $\Rightarrow$ flip sign.}
\end{ebox}

\vspace{0.3pt}
\shead{graybg}{gray}{TRIG PHASE SHIFTS \textnormal{\textit{($\theta\equiv\omega t-kz$)}}}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.25\linewidth}p{0.18\linewidth}p{0.25\linewidth}p{0.18\linewidth}@{}}
\toprule
Input & Result & Input & Result\\
\midrule
\rowcolor{rowB}$\sin(\theta{+}\pi/2)$ & $\cos\theta$ & $\cos(\theta{+}\pi/2)$ & $-\sin\theta$\\
$\sin(\theta{-}\pi/2)$ & $-\cos\theta$ & $\cos(\theta{-}\pi/2)$ & $\sin\theta$\\
\rowcolor{rowB}$\sin(\theta{\pm}\pi)$ & $-\sin\theta$ & $\cos(\theta{\pm}\pi)$ & $-\cos\theta$\\
\bottomrule
\end{tabular}}}\par}
\end{ebox}

\columnbreak

%────── COL 2 ──────
\shead{tealbg}{teal}{POLARIZATION --- GENERAL WAVE}
\begin{ebox}
\[\mathbf{E}=a_x\cos(\omega t\!-\!kz)\hat{x}+a_y\cos(\omega t\!-\!kz+\delta)\hat{y}\]
\textit{$a_x,a_y$: bounding box half-widths (V/m);\enspace$\delta$: phase diff (rad)}\\
\textit{\color{gray}$\delta=0$ or $\pi\Rightarrow$ \textbf{linear};\enspace$a_x=a_y,\,\delta=\pm\pi/2\Rightarrow$ \textbf{circular}}
\eq{Auxiliary angle}{$\psi_0=\arctan(a_y/a_x)\in(0,\pi/2)$}
\eq{Rotation angle $\gamma$ (major axis from $\hat{x}$)}{$\gamma\in[0,\pi)$}
\[\tan(2\gamma)=\tan(2\psi_0)\cos\delta\]
\textit{\color{coral}If sign$(\gamma)\!\neq\!$sign$(\delta)$: $\gamma\leftarrow\gamma\pm\tfrac{\pi}{2}$.}
\eq{Ellipticity angle $\chi$}{$\chi\in[-\pi/4,\pi/4]$}
\[\sin(2\chi)=\sin(2\psi_0)\sin\delta\]
\eq{Axial ratio $R=1/\tan|\chi|$}{$R\geq1$;\enspace$R=1$: circular;\enspace$R\to\infty$: linear}
\end{ebox}

\vspace{0.3pt}
\shead{tealbg}{teal}{$\chi$ SHAPE \& HANDEDNESS}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.30\linewidth}p{0.33\linewidth}p{0.30\linewidth}@{}}
\toprule
$\chi$ (rad) & Polarization Type & Handedness\\
\midrule
\rowcolor{rowB}$0$ & Linear & None\\
$(0,\,\pi/4)$ & RHEP & RH (CCW)\\
\rowcolor{rowB}$\pi/4$ & RHC & RH (CCW)\\
$-\pi/4$ & LHC & LH (CW)\\
\rowcolor{rowB}$(-\pi/4,\,0)$ & LHEP & LH (CW)\\
\bottomrule
\end{tabular}}}\par}
\end{ebox}

\vspace{0.3pt}
\shead{tealbg}{teal}{QUADRANT RULE FOR $\tan(2\gamma)$}
\begin{ebox}
$N=\sin(2\psi_0)\cos\delta$,\quad$D=\cos(2\psi_0)$:\\[0.5pt]
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.08\linewidth}p{0.08\linewidth}p{0.37\linewidth}p{0.37\linewidth}@{}}
\toprule
$N$ & $D$ & $2\gamma$ (deg) & $2\gamma$ (rad)\\
\midrule
\rowcolor{rowB}$+$ & $+$ & $\arctan(N/D)$ & $\arctan(N/D)$\\
$+$ & $-$ & $180°-\arctan|N/D|$ & $\pi-\arctan|N/D|$\\
\rowcolor{rowB}$-$ & $-$ & $180°+\arctan|N/D|$ & $\pi+\arctan|N/D|$\\
$-$ & $+$ & $360°-\arctan|N/D|$ & $2\pi-\arctan|N/D|$\\
\bottomrule
\end{tabular}}}\par}
\textit{$\gamma=(2\gamma)/2$.\quad$D=0\Rightarrow2\gamma=\tfrac{\pi}{2}\Rightarrow\gamma=\tfrac{\pi}{4}$.}
\end{ebox}

\vspace{0.3pt}
\shead{pinkbg}{pink}{PHASOR $\leftrightarrow$ TIME DOMAIN}
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.44\linewidth}p{0.51\linewidth}@{}}
\toprule
Phasor coeff & Time domain (×$e^{-\alpha z}$ in lossy)\\
\midrule
\rowcolor{rowB}$A$ (real) & $A\cos(\omega t-\beta z)$\\
$-jA$ & $A\sin(\omega t-\beta z)$\\
\rowcolor{rowB}$+jA$ & $-A\sin(\omega t-\beta z)$\\
$Ae^{j\phi}$ & $A\cos(\omega t-\beta z+\phi)$\\
\rowcolor{rowB}$A+jB$ & $|Z|\cos(\omega t-\beta z+\theta)$\\
\bottomrule
\end{tabular}}}\par}
$|Z|=\sqrt{A^2+B^2}$,\enspace$\theta=\arctan(B/A)$ (check quadrant)
\end{ebox}

\vspace{0.3pt}
\shead{greenbg}{green}{AVERAGE POWER DENSITY (POYNTING)}
\begin{ebox}
\eq{General}{W/m$^2$}
\[\mathbf{S}_\text{av}=\tfrac{1}{2}\operatorname{Re}\{\tilde{\mathbf{E}}\times\tilde{\mathbf{H}}^*\}\]
\eq{Lossless shortcut}{$|E_0|^2=a_x^2+a_y^2+a_z^2$}
\[\mathbf{S}_\text{av}=\frac{|E_0|^2}{2\eta}\,\hat{k}\]
\textit{Direction$=\hat{k}$. Lossy: $\mathbf{S}_\text{av}=\frac{|E(0)|^2}{2|\eta_c|}e^{-2\alpha z}\cos\theta_\eta\,\hat{k}$.}
\end{ebox}

\columnbreak

%────── COL 3 ──────
\shead{coralbg}{coral}{LOSSY MEDIA --- CLASSIFICATION}
\begin{ebox}
\eq{Loss tangent}{}
\[\frac{\varepsilon''}{\varepsilon'}=\frac{\sigma}{\omega\varepsilon_r\varepsilon_0}\]
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.25\linewidth}p{0.35\linewidth}p{0.30\linewidth}@{}}
\toprule
$\varepsilon''/\varepsilon'$ & Type & Use\\
\midrule
\rowcolor{rowB}$>100$ & Good conductor & GC approx\\
$0.01$ to $100$ & Quasi-conductor & Exact eqs\\
\rowcolor{rowB}$<0.01$ & Low-loss diel. & LD approx\\
\bottomrule
\end{tabular}}}\par}
\textit{$\varepsilon'=\varepsilon_r\varepsilon_0$;\;$\varepsilon''=\sigma/\omega$;\;nonmag.: $\mu=\mu_0$.}
\end{ebox}

\vspace{0.3pt}
\shead{coralbg}{coral}{ANY MEDIUM --- EXACT ($X=\sqrt{1+(\varepsilon''/\varepsilon')^2}$)}
\begin{ebox}
\[\alpha=\omega\sqrt{\tfrac{\mu\varepsilon'}{2}}\sqrt{X\!-\!1}\;\text{Np/m},\quad\beta=\omega\sqrt{\tfrac{\mu\varepsilon'}{2}}\sqrt{X\!+\!1}\;\text{rad/m}\]
\[\eta_c=\frac{\eta}{X^{1/2}}e^{j\theta_\eta},\quad\theta_\eta=\tfrac{1}{2}\arctan\!\tfrac{\sigma}{\omega\varepsilon'}\;\text{(rad)}\]
\textit{$\eta=\eta_0/\sqrt{\varepsilon_r}$;\enspace$\lambda=2\pi/\beta$;\enspace$u_p=\omega/\beta$.}
\end{ebox}

\vspace{0.3pt}
\shead{purplebg}{purple}{LOSSLESS ($\sigma=0$)}
\begin{ebox}
$\alpha=0$\quad$\beta=k=\omega\sqrt{\mu\varepsilon}$ (rad/m)\\
$\eta_c=\eta=\sqrt{\mu/\varepsilon}$ ($\Omega$, real;\enspace$\theta_\eta=0$)\\
$u_p=c/\sqrt{\varepsilon_r}$ (m/s);\enspace$\mathbf{E}$ and $\mathbf{H}$ in phase
\end{ebox}

\vspace{0.3pt}
\shead{coralbg}{coral}{LOW-LOSS MEDIUM ($\varepsilon''/\varepsilon'\ll1$)}
\begin{ebox}
\[\alpha\approx\tfrac{\sigma}{2}\sqrt{\tfrac{\mu}{\varepsilon'}}=\tfrac{\sigma\eta}{2}\;\text{(Np/m)}\]
\[\beta\approx\omega\sqrt{\mu\varepsilon'}=\tfrac{\omega\sqrt{\varepsilon_r}}{c}\;\text{(rad/m)}\]
\[\eta_c\approx\eta=\sqrt{\tfrac{\mu}{\varepsilon'}}\;(\Omega,\;\angle\approx0)\]
\end{ebox}

\vspace{0.3pt}
\shead{coralbg}{coral}{GOOD CONDUCTOR ($\varepsilon''/\varepsilon'\gg1$)}
\begin{ebox}
\[\alpha\approx\beta\approx\sqrt{\pi f\mu\sigma}\;\text{(Np/m, rad/m)}\]
\[\eta_c\approx(1+j)\sqrt{\tfrac{\pi f\mu}{\sigma}}\approx(1+j)\tfrac{\alpha}{\sigma}\;(\Omega)\]
\[\delta_s=\frac{1}{\alpha}=\frac{1}{\sqrt{\pi f\mu\sigma}}\;\text{(m, good cond.)}\]
\end{ebox}

\vspace{0.3pt}
\shead{bluebg}{blue}{$\tilde{\mathbf{H}}$ PHASOR $\to$ $\tilde{\mathbf{E}}$ IN LOSSY MEDIA}
\begin{ebox}
\eq{Step 1}{read $\alpha$, $\beta$ from $\tilde{\mathbf{H}}\propto e^{-\alpha y}e^{-j\beta y}$}
\eq{Step 2}{$\eta_c=\frac{j\omega\mu_0}{\gamma}=\frac{\omega\mu_0(\beta+j\alpha)}{\alpha^2+\beta^2}$}
\eq{Step 3}{$\tilde{\mathbf{E}}=-\eta_c\,(\hat{k}\times\tilde{\mathbf{H}})$}
\eq{Step 4}{$Ae^{j\phi}e^{-\alpha y}\xrightarrow{\operatorname{Re}\{\cdot e^{j\omega t}\}}Ae^{-\alpha y}\cos(\omega t-\beta y+\phi)$}
\textit{\color{coral}Convert each $A+jB\to|Z|\angle\theta$ first.}
\end{ebox}

\vspace{0.3pt}
\shead{graybg}{gray}{USEFUL NUMBERS}
\begin{ebox}
$\sqrt{2}\approx1.414$;\;$\sqrt{3}\approx1.732$;\;$\sqrt{6}\approx2.449$\\
$\sqrt{2.5}\approx1.581$;\;$\sqrt{2.56}=1.6$;\;$\sqrt{9}=3$\\
$\arctan(4/3)\approx53.1°$;\;$\arctan(3/4)\approx36.9°$\\
$\arccos(2/3)\approx48.2°$;\;$\arctan(24/7)\approx73.7°$
\end{ebox}

\end{multicols}

\vfill
\noindent
\begin{minipage}[t]{0.19\linewidth}
\shead{purplebg}{purple}{WAVE PARAMS}
\begin{ebox}
$\omega=2\pi f$ --- ang.\ freq (rad/s)\\
$k=\omega\sqrt{\varepsilon_r}/c$ --- wavenumber\\
$\lambda=2\pi/k$ --- wavelength (m)\\
$u_p=c/\sqrt{\varepsilon_r}$ --- phase vel.\ (m/s)\\
$\eta=\eta_0/\sqrt{\varepsilon_r}$ --- impedance ($\Omega$)\\
$c=3\times10^8$ m/s;\;$\eta_0=377\,\Omega$
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{purplebg}{purple}{LOSSLESS MEDIA}
\begin{ebox}
$\sigma=0$ (ideal dielectric)\\
$\alpha=0$;\enspace no attenuation\\
$\eta=\eta_0/\sqrt{\varepsilon_r}$ (real, $\Omega$)\\
$\mathbf{E}$ and $\mathbf{H}$ in phase ($\theta_\eta=0$)\\
$S_\text{av}=|E_0|^2/(2\eta)$ (W/m$^2$)
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{coralbg}{coral}{LOSSY MEDIA}
\begin{ebox}
$\alpha$ --- atten.\ const (Np/m)\\
$\beta$ --- phase const (rad/m)\\
$\gamma=\alpha+j\beta$ --- prop.\ const\\
$\eta_c$ --- complex impedance ($\Omega$)\\
$\delta_s=1/\alpha$ --- skin depth (m)\\
$\varepsilon'=\varepsilon_r\varepsilon_0$;\;$\varepsilon''=\sigma/\omega$
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{amberbg}{amber}{CIRCULAR WAVES}
\begin{ebox}
RHC: $\tilde{\mathbf{E}}=a(\hat{x}-j\hat{y})e^{-jkz}$\\
LHC: $\tilde{\mathbf{E}}=a(\hat{x}+j\hat{y})e^{-jkz}$\\
Linear: $a_R=a_L=a_x/2$\\
$-j=e^{-j\pi/2}$: $\cos\to\sin$\\
$+j=e^{+j\pi/2}$: $\cos\to-\sin$
\end{ebox}
\end{minipage}\hfill
\begin{minipage}[t]{0.19\linewidth}
\shead{tealbg}{teal}{POLARIZATION}
\begin{ebox}
Linear: $\delta=0$ or $\pi$\\
LHC: $a_x\!=\!a_y$, $\delta\!=\!+\pi/2$\\
RHC: $a_x\!=\!a_y$, $\delta\!=\!-\pi/2$\\
$R=1$: circular;\;$R\to\infty$: linear\\
$\chi>0$: RH;\;$\chi<0$: LH
\end{ebox}
\end{minipage}

\end{document}
