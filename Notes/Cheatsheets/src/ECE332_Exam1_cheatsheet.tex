...
% image path for pol_ellipse.jpg (lives two levels up from src/)
\graphicspath{{../../}}
...
\begin{ebox}
{\centering\makebox[0.98\linewidth][c]{{\setlength{\tabcolsep}{2pt}\begin{tabular}{@{}p{0.19\linewidth}p{0.36\linewidth}p{0.38\linewidth}@{}}
\toprule
& Lossless ($\sigma\!=\!0$) & Lossy ($\sigma\!\ne\!0$)\\
\midrule
\rowcolor{rowB}Impedance & $\eta=\sqrt{\mu/\varepsilon}$ \textit{(real)} & $\eta_c=|\eta_c|e^{j\theta_\eta}$ \textit{(cmplx)}\\
Prop.\ factor & $e^{-jkz}$ & $e^{-\alpha z}e^{-j\beta z}$\\
\rowcolor{rowB}$\mathbf{E}(t)$ & \multicolumn{2}{l}{$\operatorname{Re}\{\tilde{\mathbf{E}}\,e^{j\omega t}\}$ \quad (same for both)}\\
$\mathbf{H}(t)$ & \multicolumn{2}{l}{$\operatorname{Re}\{\tilde{\mathbf{H}}\,e^{j\omega t}\}$ \quad (same for both)}\\
...
\rowcolor{rowB}E,\,H phase & in phase & H lags E by $\theta_\eta$\\
\midrule
\multicolumn{3}{l}{\textit{Lossless shortcuts (wave in $+z$, $\mathbf{E}$ along $\hat{e}$, amplitude $E_0$):}}\\
\rowcolor{rowB}$\mathbf{E}(t)$ & \multicolumn{2}{l}{$E_0\cos(\omega t-kz+\phi)\,\hat{e}$}\\
$\mathbf{H}(t)$ & \multicolumn{2}{l}{$\tfrac{E_0}{\eta}\cos(\omega t-kz+\phi)\,(\hat{k}\times\hat{e})$}\\
\bottomrule
\end{tabular}}}\par}
\end{ebox}
...
\begin{ebox}
$\alpha=0$\quad$\beta=\omega\sqrt{\mu\varepsilon}=k$ (rad/m)\quad$\lambda=u_p/f$ (m)\\
$\eta_c=\eta=\sqrt{\mu/\varepsilon}$ ($\Omega$, real;\enspace$\theta_\eta=0$)\\
$u_p=1/\sqrt{\mu\varepsilon}=c/\sqrt{\varepsilon_r}$ (m/s)
\end{ebox}
