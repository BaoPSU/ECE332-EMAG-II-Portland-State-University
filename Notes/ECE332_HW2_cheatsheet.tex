...
\begin{ebox}
\textit{\color{coral}$\delta$ gatekeeper: $\delta\!=\!0,\pi\Rightarrow$\textbf{linear};\enspace$\delta\!=\!\pm\pi/2$\,\&\,$a_x\!=\!a_y\Rightarrow$\textbf{circular};\enspace else \textbf{elliptical}.}\\[0.5pt]
{\centering\begin{tabular}{@{}p{0.22\linewidth}p{0.42\linewidth}p{0.22\linewidth}@{}}
\toprule
Type & Condition & $\chi$\\
\midrule
...
\textit{$\chi>0$: RH/CW.\enspace$\chi<0$: LH/CCW.\enspace$|\chi|=45^\circ$: circ.\enspace$\chi=0$: linear.}\\
\textit{$\gamma$ tilt: $0\!=\!\to$;\enspace$\pi/4\!=\!45°$;\enspace$\pi/2\!=\!\uparrow$;\enspace$(0,\pi/4)$: horiz side;\enspace$(\pi/4,\pi/2)$: vert side. Same sign as $\cos\delta$.}
\end{ebox}
...
\makebox[0.92\linewidth][c]{\begin{tabular}{@{}cccll@{}}
\toprule
$N$ & $D$ & Q & $2\gamma$ (deg) & $2\gamma$ (rad)\\
\midrule
\rowcolor{rowB}$+$ & $+$ & 1 & $\arctan(N/D)$ & $\arctan(N/D)$\\
$+$ & $-$ & 2 & $180^\circ-\arctan|N/D|$ & $\pi-\arctan|N/D|$\\
\rowcolor{rowB}$-$ & $-$ & 3 & $180^\circ+\arctan|N/D|$ & $\pi+\arctan|N/D|$\\
$-$ & $+$ & 4 & $360^\circ-\arctan|N/D|$ & $2\pi-\arctan|N/D|$\\
\bottomrule
\end{tabular}}\\[1pt]
\textit{$\gamma=2\gamma/2$.\quad $D=0\Rightarrow2\gamma=90^\circ(=\pi/2\text{ rad})\Rightarrow\gamma=45^\circ$.}
\end{ebox}
...
\textit{Linear $\Rightarrow$ $a_R=a_L=a_x/2$.\enspace$-j=e^{-j\pi/2}$: cos$\to$sin;\enspace$+j$: cos$\to{-}$sin.}
\end{ebox}

\vspace{1pt}
\shead{pinkbg}{pink}{PHASOR $\leftrightarrow$ TIME DOMAIN}
\begin{ebox}
{\centering\begin{tabular}{@{}p{0.4\linewidth}p{0.52\linewidth}@{}}
\toprule
...
$0.01<\varepsilon''/\varepsilon'<100$ & Quasi-conductor & Exact eqs\\
...
\begin{ebox}
\[\alpha\approx\omega\sqrt{\tfrac{\mu_0\varepsilon''}{2}}=\omega\sqrt{\tfrac{\mu_0\sigma}{2\omega}}=\sqrt{\pi f\mu_0\sigma}\quad\text{(Np/m)}\]
\[\beta\approx\alpha\approx\sqrt{\pi f\mu_0\sigma}\quad\text{(rad/m)}\]
\[u_p=\omega/\beta\;\text{(m/s)},\quad\lambda=2\pi/\beta\;\text{(m)}\]
\[\eta_c\approx\sqrt{j\tfrac{\mu_0}{\varepsilon''}}=(1+j)\sqrt{\tfrac{\pi f\mu_0}{\sigma}}\approx(1+j)\tfrac{\alpha}{\sigma}\quad(\Omega)\]
\end{ebox}

\vspace{1pt}
\shead{coralbg}{coral}{LOW-LOSS DIELECTRIC ($\sigma/\omega\varepsilon\ll 0.01$)}
\begin{ebox}
\[\alpha\approx\tfrac{\omega\varepsilon''}{2}\sqrt{\tfrac{\mu_0}{\varepsilon'}}=\tfrac{\sigma}{2}\sqrt{\tfrac{\mu_0}{\varepsilon}}=\tfrac{\sigma\eta}{2}\;\text{(Np/m)}\]
\[\beta\approx\omega\sqrt{\mu_0\varepsilon}=\tfrac{\omega\sqrt{\varepsilon_r}}{c}\;\text{(rad/m)},\quad u_p\approx\tfrac{c}{\sqrt{\varepsilon_r}}\;\text{(m/s)}\]
\[\eta_c\approx\sqrt{\tfrac{\mu_0}{\varepsilon}}=\eta\;(\Omega,\;\angle\approx0^\circ)\]
\end{ebox}
