...
where $\eta=\eta_0/\sqrt{\varepsilon_r}$.\quad$\eta_c=|\eta_c|\angle\theta_\eta$.
\end{ebox}

\vspace{1pt}
\shead{coralbg}{coral}{GOOD CONDUCTOR ($\sigma/\omega\varepsilon\gg 100$)}
\begin{ebox}
\[\alpha\approx\beta\approx\sqrt{\pi f\mu_0\sigma}\quad\text{(Np/m, rad/m)}\]
\[u_p=\sqrt{4\pi f/\mu_0\sigma}=\omega/\beta\;\text{(m/s)},\quad\lambda=2\pi/\beta\;\text{(m)}\]
...
\begin{ebox}
\[\alpha\approx\tfrac{\sigma\eta}{2}\;\text{(Np/m)},\quad\beta\approx\tfrac{\omega\sqrt{\varepsilon_r}}{c}\;\text{(rad/m)}\]
\[u_p\approx\tfrac{c}{\sqrt{\varepsilon_r}}\;\text{(m/s)},\quad\eta_c\approx\eta\;(\Omega,\;\angle\approx0^\circ)\]
\end{ebox}
