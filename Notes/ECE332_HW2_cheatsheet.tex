...
\midrule
\rowcolor{rowB}$\psi=0$ & Horizontal ($\rightarrow$), aligned with $\hat{x}$\\
$0 < \psi < \pi/4$ & Tilted — horiz side\\
\rowcolor{rowB}$\psi=\pi/4$ & 45° diagonal\\
$\pi/4 < \psi < \pi/2$ & Tilted — vert side\\
\rowcolor{rowB}$\psi=\pi/2$ & Vertical ($\uparrow$), aligned with $\hat{y}$\\
\bottomrule
\end{tabular}\par}
\end{ebox}
...
{\centering\begin{tabular}{@{}p{0.22\linewidth}p{0.32\linewidth}p{0.32\linewidth}@{}}
\toprule
$\chi$ (rad) & Polarization Type & Handedness\\
\midrule
\rowcolor{rowB}$\chi=0$ & Linear & None\\
$0 < \chi < \pi/4$ & RHEP & RH (CW)\\
\rowcolor{rowB}$\chi=\pi/4$ & RHC & RH (CW)\\
$\chi=-\pi/4$ & LHC & LH (CCW)\\
\rowcolor{rowB}$-\pi/4 < \chi < 0$ & LHEP & LH (CCW)\\
\bottomrule
\end{tabular}\par}
\end{ebox}
