...
$\pi/2$ & Vertical ($\uparrow$), aligned with $\hat{y}$\\
\bottomrule
\end{tabular}\par}\vspace{0.5pt}
\textit{$(0,\pi/4)$: tilted horiz side;\enspace$(\pi/4,\pi/2)$: tilted vert side.}\\[-0.5pt]
\end{ebox}

\vspace{0.3pt}
\shead{tealbg}{teal}{$\chi$ SHAPE \& HANDEDNESS}
\begin{ebox}
\textit{$\chi$ describes the shape of the ellipse and handedness (rotation direction).}\\[1pt]
