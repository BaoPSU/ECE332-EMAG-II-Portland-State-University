...
\fancyhead[L]{\fontsize{6.5}{7}\selectfont\textbf{ECE332 --- HW2 Equation Sheet (v2)}}
...
\begin{ebox}
{\centering\includegraphics[width=\linewidth,keepaspectratio]{pol_ellipse.jpg}\par}
\[\mathbf{E}=a_x\cos(\omega t\!-\!kz)\hat{x}+a_y\cos(\omega t\!-\!kz+\delta)\hat{y}\]
\textit{$a_x,a_y$: bounding box half-widths (V/m);\enspace$\delta$: phase diff (rad)}\\[-1pt]
\textit{\color{gray}$\delta=0$ or $\pi\Rightarrow$ \textbf{linear} pol.\enspace($\chi=0$, no ellipse)}
...
%──────────────────────────────────────
% TABLE 7-1: full-width, replaces EXACT FORMULAS / GOOD CONDUCTOR / LOW-LOSS DIELECTRIC
%──────────────────────────────────────
\vspace{2pt}
{\fontsize{5.3}{6.8}\selectfont
\noindent{\centering\textbf{Table~7-1}\enspace
  Expressions for $\alpha$, $\beta$, $\eta_c$, $u_p$, and $\lambda$ for various types of media.\par}
\vspace{1.5pt}
...
\begin{tabular}{@{}
  >{\bfseries}l
  >{\centering\arraybackslash}m{0.295\textwidth}
...
\toprule
\rowcolor{bluebg}
 & \multicolumn{1}{c}{\color{blue}\bfseries Any Medium}
 & \color{blue}\bfseries Lossless Medium
 & \color{blue}\bfseries Low-loss Medium
 & \color{blue}\bfseries Good Conductor
 & \\[-2pt]
\rowcolor{bluebg}
 &
 & \color{blue}($\sigma=0$)
 & \color{blue}($\varepsilon''/\varepsilon'\ll 1$)
 & \color{blue}($\varepsilon''/\varepsilon'\gg 1$)
 & \color{blue}\bfseries Units \\
\midrule
$\alpha=$ &
  $\omega\!\left[\dfrac{\mu\varepsilon'}{2}\!\left(\sqrt{1\!+\!\left(\dfrac{\varepsilon''}{\varepsilon'}\right)^{\!2}}\!-\!1\right)\right]^{\!1/2}$ &
  $0$ &
  $\approx\dfrac{\sigma}{2}\sqrt{\dfrac{\mu}{\varepsilon'}}$ &
  $\approx\sqrt{\pi f\mu\sigma}$ &
  (Np/m) \\[4pt]
\rowcolor{rowB}
$\beta=$ &
  $\omega\!\left[\dfrac{\mu\varepsilon'}{2}\!\left(\sqrt{1\!+\!\left(\dfrac{\varepsilon''}{\varepsilon'}\right)^{\!2}}\!+\!1\right)\right]^{\!1/2}$ &
  $\omega\sqrt{\mu\varepsilon}$ &
  $\approx\omega\sqrt{\mu\varepsilon'}$ &
  $\approx\sqrt{\pi f\mu\sigma}$ &
  (rad/m) \\[4pt]
$\eta_c=$ &
  $\sqrt{\dfrac{\mu}{\varepsilon'}}\!\left(1\!-\!j\dfrac{\varepsilon''}{\varepsilon'}\right)^{\!-1/2}$\newline
  \textit{or:}\enspace$\eta/X^{1/2}\angle\theta_\eta$,\;$\theta_\eta\!=\!\tfrac{1}{2}\arctan\!\tfrac{\sigma}{\omega\varepsilon'}$ &
  $\sqrt{\dfrac{\mu}{\varepsilon}}$ &
  $\approx\sqrt{\dfrac{\mu}{\varepsilon'}}$ &
  $\approx(1\!+\!j)\dfrac{\alpha}{\sigma}$ &
  ($\Omega$) \\[4pt]
\rowcolor{rowB}
$u_p=$ &
  $\omega/\beta$ &
  $1/\sqrt{\mu\varepsilon}$ &
  $\approx 1/\sqrt{\mu\varepsilon'}$ &
  $\approx\sqrt{4\pi f/\mu\sigma}$ &
  (m/s) \\
$\lambda=$ &
  $2\pi/\beta = u_p/f$ &
  $u_p/f$ &
...
\multicolumn{6}{@{}l@{}}{\textit{Notes:
  $\varepsilon'=\varepsilon_r\varepsilon_0$;\enspace
  $\varepsilon''=\sigma/\omega$;\enspace
  $X=\sqrt{1+(\varepsilon''/\varepsilon')^2}$;\enspace
  $\eta=\eta_0/\sqrt{\varepsilon_r}$;\enspace
  in free space $\varepsilon=\varepsilon_0$, $\mu=\mu_0$;\enspace
  low-loss: $\varepsilon''/\varepsilon'<0.01$;\enspace
  GC: $\varepsilon''/\varepsilon'>100$.}}\\
\end{tabular}%
}
}
